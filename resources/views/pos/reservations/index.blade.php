...
@section('content')
    ...
    <div class="ops-page">
        ...
        @else
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                        ...
                                        <div class="reservation-preorder__items">
                                            @foreach ($reservation->preorder->items as $item)
                                                <article class="reservation-preorder__item">
                                                    ...
                                                    @endif
                                                    <div><strong>{{ $item->product_name }}@if($item->variant_name) — {{ $item->variant_name }}@endif</strong><span>Số lượng:
                                ...
                                @if (in_array(
                                        $reservation->status,
                                        [\App\Enums\ReservationStatus::Pending, \App\Enums\ReservationStatus::Confirmed, \App\Enums\ReservationStatus::Late],
                                        true))
                                        ...
                                        <form method="post"
                                                    ...
                                                    <div class="reservation-editor__item">
                                                        <select class="form-select"
                                                            ...
                                                            @foreach ($products as $product)
                                                                <option value="{{ $product->id }}" data-variants='@json($product->availableVariants->map->only(["id", "name", "price"]))'
                                                                    @selected($editItem->product_id === $product->id)>{{ $product->name }} ·
                                                                    {{ $product->display_price }}</option>
                                                            @endforeach
                                                        </select>
                                                        <select class="form-select" name="items[{{ $index }}][variant_id]" data-reservation-variant
                                                            @disabled($products->firstWhere('id', $editItem->product_id)?->availableVariants->isEmpty())
                                                            @if($products->firstWhere('id', $editItem->product_id)?->availableVariants->isEmpty()) hidden @else required @endif>
                                                            @foreach($products->firstWhere('id', $editItem->product_id)?->availableVariants ?? [] as $variant)
                                                                <option value="{{ $variant->id }}" @selected($editItem->product_variant_id === $variant->id)>{{ $variant->name }} · {{ number_format($variant->price) }} ₫</option>
                                                            @endforeach
                                                        </select>
                                            ...
                                            <template data-reservation-item-template>
                                                <div class="reservation-editor__item"><select class="form-select"
                                                    ...
                                                    <select class="form-select" data-name="variant_id" data-reservation-variant hidden disabled></select>
                            ...
                            <div class="modal-footer reservation-modal__actions">
                                ...
                                @endif
                                @if (in_array($reservation->status, [\App\Enums\ReservationStatus::Confirmed, \App\Enums\ReservationStatus::Late], true))
                                    <button class="btn btn-outline-primary" type="button" data-open-reservation-edit
                                        data-bs-toggle="collapse"
                                        data-bs-target="#reservation-editor-{{ $reservation->id }}" aria-expanded="false"
                                ...
                                @endif
                                @if (
                                    in_array($reservation->status, [\App\Enums\ReservationStatus::Confirmed, \App\Enums\ReservationStatus::Late], true) &&
                                ...
                                @endif
                                @if (in_array($reservation->status, [\App\Enums\ReservationStatus::Confirmed, \App\Enums\ReservationStatus::Late], true) && auth()->user()->can('reservation.mark-no-show'))
                                    <form method="post"
                                ...
                                @endif
                                @if (in_array($reservation->status, [\App\Enums\ReservationStatus::Pending, \App\Enums\ReservationStatus::Confirmed, \App\Enums\ReservationStatus::Late], true))
                                    <details class="reservation-cancel">
                                        <summary class="btn btn-outline-danger">Hủy đặt bàn</summary>
                                        <form method="post" action="{{ route($reservationRoute . '.cancel', $reservation) }}">
                                            @csrf @method('patch')
                                            <input class="form-control" name="reason" maxlength="500" required
                                                placeholder="Nhập lý do hủy">
                                            <button class="btn btn-danger">Xác nhận hủy</button>
                                        </form>
                                    </details>
                                @endif
                                @if ($reservation->diningSession)
                                ...
                                @if ($reservation->status === \App\Enums\ReservationStatus::Cancelled)
                                    <div class="reservation-modal__empty">
                                        ...
                                        @if ($reservation->cancelledBy)
                                            <small>Thực hiện bởi {{ $reservation->cancelledBy->name }} lúc {{ $reservation->cancelled_at?->format('H:i · d/m/Y') }}</small>
                                        @endif
                                    </div>
                                @endif
            ...
            @endforeach
            <script>
                const refreshReservationVariant = row => {
                ...
                document.addEventListener('change', event => {
                    if (event.target.matches('.reservation-editor__item [name$="[product_id]"]')) refreshReservationVariant(event.target.closest('.reservation-editor__item'));
                });
