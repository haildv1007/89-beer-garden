    ...
    <div class="operation-record-page reservation-record-page">
        ...
        <dl class="operation-record-facts">
            ...
            @if ($reservation->status === \App\Enums\ReservationStatus::Cancelled)
                ...
                <div>
                    <dt>Người hủy</dt>
                    <dd>{{ $reservation->cancelledBy?->name ?: '—' }} · {{ $reservation->cancelled_at?->format('H:i d/m/Y') }}</dd>
                </div>
            @endif
        </dl>
        ...
        <div class="d-flex flex-wrap gap-2">
            @if ($reservation->status === \App\Enums\ReservationStatus::Pending)
                ...
                @endif @if (in_array($reservation->status, [\App\Enums\ReservationStatus::Confirmed, \App\Enums\ReservationStatus::Late], true) && auth()->user()->can('reservation.mark-no-show'))
                ...
                @if (in_array($reservation->status, [\App\Enums\ReservationStatus::Pending, \App\Enums\ReservationStatus::Confirmed, \App\Enums\ReservationStatus::Late], true))
                    <details class="reservation-cancel">
                        ...
                        <form class="mt-2 d-flex gap-2" method="post"
                            action="{{ route($reservationRoute . '.cancel', $reservation) }}">
                            @csrf @method('patch')
