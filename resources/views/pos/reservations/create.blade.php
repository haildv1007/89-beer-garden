        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const refresh = () => {
                ...
                items.querySelectorAll('.manual-item-row').forEach(row => {
                    ...
                    sum += Number((!variant.disabled && variant.selectedOptions[0]?.dataset.price) || product.selectedOptions[0]?.dataset.price || 0) * quantity
                });
            ...
            items.addEventListener('change', e => { if (e.target.name?.endsWith('[product_id]')) refreshVariant(e.target.closest('.manual-item-row')); refresh() })
        });
    </script>
@endsection
