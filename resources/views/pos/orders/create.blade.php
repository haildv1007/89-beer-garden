                            ...
                            @forelse($products as $index => $product)
                                <tr
                                    ...
                                    <td>
                                        ...
                                        @if($product->variants->isNotEmpty())<select class="form-select mt-2" name="items[{{ $index }}][variant_id]" required disabled data-order-field="{{ $index }}"><option value="">Chọn set</option>@foreach($product->variants->where('is_available', true) as $variant)<option value="{{ $variant->id }}">{{ $variant->name }} · {{ number_format($variant->price, 0, ',', '.') }} ₫</option>@endforeach</select>@endif
                                    </td>
