...
@php
    ...
    $defaultSiteName = __('app.name');
    $defaultTitleSuffix = ' - ' . $defaultSiteName;
    $pageTitle = str_ends_with($declaredTitle, $defaultTitleSuffix)
        ? substr($declaredTitle, 0, -strlen($defaultTitleSuffix))
        : $declaredTitle;
    $documentTitle = request()->routeIs('customer.home')
        ? ($publicSettings[\App\Services\SystemSetting\SystemSettingCatalog::SEO_TITLE] ??
            $siteName . ' - ' . __('customer_ui.brand_tagline'))
        : ($pageTitle !== ''
            ? $pageTitle . ' - ' . $siteName
            : ($publicSettings[\App\Services\SystemSetting\SystemSettingCatalog::SEO_TITLE] ?? $siteName));
    $metaDescription =
    ...
    $storedAssetUrl = static fn(string $key): ?string => isset($publicSettings[$key])
        ...
        : null;
    $siteLogoUrl =
        $storedAssetUrl(\App\Services\SystemSetting\SystemSettingCatalog::SITE_LOGO) ??
    ...
    $faviconUrl = $storedAssetUrl(\App\Services\SystemSetting\SystemSettingCatalog::SITE_FAVICON) ?? $siteLogoUrl;
...
<head>
    ...
    @if ($publicSettings[\App\Services\SystemSetting\SystemSettingCatalog::SEO_KEYWORDS] ?? null)
        <meta name="keywords"
            content="{{ $publicSettings[\App\Services\SystemSetting\SystemSettingCatalog::SEO_KEYWORDS] }}">
    @endif
    @if ($faviconUrl)
        <link rel="icon" href="{{ $faviconUrl }}">
    @endif
    @if (request()->routeIs('customer.home'))
        <link rel="preload" as="image" href="{{ $siteBannerUrl ?? asset('images/beer-garden-hero.png') }}"
            fetchpriority="high">
    @endif
    ...
    @unless (request()->routeIs('customer.posts.show'))
        ...
        <meta property="og:title"
            content="{{ $publicSettings[\App\Services\SystemSetting\SystemSettingCatalog::OG_TITLE] ?? $documentTitle }}">
        @if ($publicSettings[\App\Services\SystemSetting\SystemSettingCatalog::OG_DESCRIPTION] ?? $metaDescription)
            <meta property="og:description"
                content="{{ $publicSettings[\App\Services\SystemSetting\SystemSettingCatalog::OG_DESCRIPTION] ?? $metaDescription }}">
        @endif
...
<body>
        ...
        <nav class="navbar navbar-expand-lg navbar-light site-navbar" aria-label="{{ __('customer_ui.navigation') }}">
            <div class="container">
                ...
                <a class="brand-lockup" href="{{ route('customer.home') }}" aria-label="{{ $siteName }}">
                    <img class="brand-mark" src="{{ $siteLogoUrl }}" alt="" aria-hidden="true">
                    <span
                        class="brand-wordmark">{{ $siteName }}<small>{{ __('customer_ui.brand_tagline') }}</small></span>
                </a>
                <div class="mobile-header-actions d-lg-none">
                    ...
                    <button class="navbar-toggler" type="button" data-bs-toggle="collapse"
                        ...
                        aria-label="{{ __('customer_ui.navigation') }}"><span
                    ...
                    <ul class="navbar-nav mx-lg-auto gap-lg-2">
                        ...
                        <li class="nav-item"><a @class([
                            ...
                            'active' => request()->routeIs('customer.posts.*'),
                                ...
                                href="{{ route('customer.posts.index') }}">{{ __('customer_ui.news_navigation') }}</a>
                        </li>
                        <li class="nav-item">
                            <a @class([
                                'nav-link',
                                'nav-reservation',
                                'active' => request()->routeIs('customer.reservations.*'),
                            ])
                            ...
                            <div class="dropdown"><button class="btn btn-light-outline btn-sm dropdown-toggle"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    ...
                                    @can('customer.order.view-own')
                                        <li><a class="dropdown-item"
                                                href="{{ route('customer.fulfillment-orders.index') }}">Lịch sử đặt món</a>
                                        </li>
                                        <li><a class="dropdown-item"
    ...
    <nav class="mobile-bottom-nav" aria-label="Điều hướng nhanh">
        ...
        <button @class([
            'mobile-bottom-nav__item',
            'active' => request()->routeIs('customer.cart.*', 'customer.checkout.*'),
        ]) type="button" data-mini-cart-mobile aria-expanded="false"
            aria-controls="miniCartPanel" aria-haspopup="dialog" aria-label="{{ __('customer_order.mini_open') }}">
    ...
    <footer class="site-footer">
        <div class="container">
            <div class="footer-main">
                ...
                <nav class="footer-nav" aria-labelledby="footer-explore-title">
                    ...
                    <ul class="footer-links">
                        ...
                        <li><a
                                href="{{ route('customer.contact') }}">{{ __('customer_ui.contact_navigation') }}</a>
                        </li>
                    </ul>
                </nav>
                ...
                <nav class="footer-nav" aria-labelledby="footer-account-title">
                    ...
                    <ul class="footer-links">
                        ...
                        @else
                            ...
                            @endcan
                            @can('customer.order.view-own')
                                <li><a href="{{ route('customer.fulfillment-orders.index') }}">Lịch sử đặt món</a></li>
                                <li><a href="{{ route('customer.orders.history') }}">{{ __('order_history.title') }}</a>
                                </li>
                            @endcan
                        @endguest
                    </ul>
            ...
            <div class="footer-bottom">
                <span>{{ __('customer_ui.copyright', ['year' => now()->year, 'name' => $siteName]) }}</span>
                <span>{{ __('customer_ui.brand_tagline') }}</span>
            </div>
        </div>
    </footer>
    @unless (request()->routeIs('customer.menu.*'))
        @include('customer.partials.product-quick-view')
    @endunless
    @if ($aiChatEnabled ?? config('features.ai_chat'))
        @include('customer.partials.ai-chat-widget')
    @endif
