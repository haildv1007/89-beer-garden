...
<body>
    ...
    <header>
        <div class="type {{ $kitchenTicket->type !== 'order' ? 'danger' : '' }}">
            {{ ($kitchenTicket->type === 'cancellation' && ($p['cancellation_scope'] ?? null) === 'order') ? 'PHIẾU HỦY LƯỢT' : ['order' => 'PHIẾU BẾP', 'adjustment' => 'PHIẾU ĐIỀU CHỈNH', 'cancellation' => 'PHIẾU HỦY MÓN'][$kitchenTicket->type] }}
        </div>
        <h1>{{ $p['location'] ?? '' }}</h1>
    ...
    <section class="items">
        @foreach ($p['items'] ?? [] as $item)
            <div class="item"><strong>{{ $item['quantity'] }} × {{ $item['product_name'] }}@if(!empty($item['variant_name'])) — {{ $item['variant_name'] }}@endif</strong>
                @if ($item['note'] ?? null)
