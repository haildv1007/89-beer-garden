        ...
        <div class="row g-0">
            ...
            <div class="col-lg-7">
                <div class="form-body auth-form-body">
                    ...
                    <p class="text-secondary mb-4">{{ __('auth.login_copy') }}</p>
                    ...
                    <form class="js-submit-once" method="post" action="{{ route('login') }}" novalidate>@csrf
                        ...
                        <div class="mb-2"><label for="password" class="form-label">{{ __('auth.labels.password') }} <span
                    ...
                    <div class="d-flex flex-wrap justify-content-between gap-2 mt-4">
                        <a class="auth-secondary-link"
                            href="{{ route('customer.registration.create') }}">{{ __('customer.registration.login_prompt') }}</a>
                        <a class="auth-secondary-link"
                            href="{{ route('password.request') }}">{{ __('auth.password_reset.forgot') }}</a>
                    </div>
                </div>
            </div>
        </div>
