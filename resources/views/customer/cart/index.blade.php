    ...
    @else
        <div class="cart-layout">
            <section class="cart-lines" aria-label="{{ __('customer_order.cart_items') }}">
                ...
                @foreach ($rows as $row)
                    <article class="cart-line" data-cart-line="{{ $row['line_key'] }}">
                        ...
                        <div class="cart-line-main"><span
                            ...
                            <h2>{{ $row['name'] }}</h2>@if($row['variant_name'])<small class="cart-line-variant">{{ $row['variant_name'] }}</small>@endif<span
                            ...
                            <details class="cart-note" @if ($row['note']) open @endif>
                                ...
                                <form class="cart-note-form js-submit-once" method="post"
                                    action="{{ route('customer.cart.items.update', $row['line_key']) }}">@csrf
                                    @method('patch')<input type="hidden" name="quantity"
                                        value="{{ $row['quantity'] }}"><input id="note-{{ str_replace(':', '-', $row['line_key']) }}"
                        ...
                        <form class="cart-quantity-form" method="post"
                            ...
                            <div class="cart-quantity" aria-label="{{ __('customer_order.quantity') }}"><button
                                    ...
                                    id="quantity-{{ str_replace(':', '-', $row['line_key']) }}" type="number" min="1" max="1000"
                        ...
                        <div class="cart-line-end"><strong class="cart-line-total"
                            ...
                            <form class="cart-remove-form" method="post"
                                action="{{ route('customer.cart.items.destroy', $row['line_key']) }}"
            ...
            <aside class="cart-checkout">
                ...
                <form method="post" action="{{ route('customer.cart.fulfillment.select') }}" data-fulfillment-form>@csrf
                    <fieldset class="fulfillment-picker">
                        ...
                        <label @class(['fulfillment-option', 'is-selected' => $summary['fulfillment_type'] === 'at_table'])>
                            <input type="radio" name="fulfillment_type" value="at_table"
                                @checked($summary['fulfillment_type'] === 'at_table')>
                            <span><strong>{{ __('customer_order.fulfillment.at_table') }}</strong>
                                <small>{{ __('customer_order.fulfillment.at_table_short') }}</small></span>
                        </label>
                        <label @class(['fulfillment-option', 'is-selected' => $summary['fulfillment_type'] === 'dine_in'])>
                            <input type="radio" name="fulfillment_type" value="dine_in"
                                @checked($summary['fulfillment_type'] === 'dine_in')>
                            <span><strong>{{ __('customer_order.fulfillment.dine_in') }}</strong>
                                <small>{{ __('customer_order.fulfillment.dine_in_short') }}</small></span>
                        </label>
                        @php($takeawaySelected = in_array($summary['fulfillment_type'], ['pickup', 'delivery'], true))
                        <button class="fulfillment-option fulfillment-option--parent {{ $takeawaySelected ? 'is-selected' : '' }}"
                            type="button" data-takeaway-toggle aria-expanded="{{ $takeawaySelected ? 'true' : 'false' }}"
                            aria-controls="takeaway-methods">
                            <span class="fulfillment-option__radio" aria-hidden="true"></span>
                            <span><strong>{{ __('customer_order.fulfillment.takeaway') }}</strong>
                                <small>{{ __('customer_order.fulfillment.takeaway_short') }}</small></span>
                        </button>
                        <div id="takeaway-methods" class="takeaway-methods" data-takeaway-methods @if (! $takeawaySelected) hidden @endif>
                            @foreach (['delivery', 'pickup'] as $type)
                                <label @class(['fulfillment-option', 'fulfillment-option--child', 'is-selected' => $summary['fulfillment_type'] === $type])>
                                    <input type="radio" name="fulfillment_type" value="{{ $type }}" @checked($summary['fulfillment_type'] === $type)>
                                    <span class="fulfillment-option__child-label"><strong>{{ __('customer_order.fulfillment.' . $type) }}</strong></span>
                                </label>
                            @endforeach
                        </div>
                    </fieldset>
                ...
                <div data-at-table-checkout @class([
                    ...
                    <div><strong>{{ __('customer_order.fulfillment.at_table_handoff') }}</strong>
                        <small>{{ __('customer_order.fulfillment.at_table_handoff_help') }}</small></div>
                </div>
