...
@section('content')
    <div class="account-page">
        ...
        <header class="account-heading account-heading--detail">
            <div><h1>Chi tiết đặt bàn</h1>
                <p class="reservation-detail-code">Mã đặt bàn <span>{{ $reservation->reservation_code }}</span>
                </p>
            </div>
            ...
            @if ($reservation->preorder)
                <div class="reservation-preorder">
                    ...
                    @foreach ($reservation->preorder->items as $item)
                        <div><span>{{ $item->product_name }}@if($item->variant_name) — {{ $item->variant_name }}@endif <small>×
