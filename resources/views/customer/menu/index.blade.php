...
@section('content')
    <div class="menu-page">
        ...
        <section class="menu-discovery" aria-label="{{ __('app.search') }}">
            <div class="sticky-categories">@include('customer.menu._categories', ['categories' => $navigationCategories, 'tags' => $tags])</div>
        </section>

        <div @class([
            'menu-results-heading',
            'menu-results-heading--quiet' => !(
                ($filters['q'] ?? null) || ($filters['category'] ?? null) || ($filters['tag'] ?? null)
            ),
        ]) id="menu-catalogue">
            <div><span class="menu-results-kicker">89 BEER GARDEN</span>
                <p>{{ __('customer_ui.results', ['count' => $products->count()]) }}</p>
            </div>
            @if (($filters['q'] ?? null) || ($filters['category'] ?? null) || ($filters['tag'] ?? null))
                ...
                <div class="product-modal-layout">
                    <div class="product-modal-gallery" data-product-modal-gallery>
                        <figure class="product-modal-media"><img data-product-modal-image src="" alt=""
                                ...
                                hidden></video><span class="product-modal-status" data-product-modal-status></span><div
                                class="product-modal-tags" data-product-modal-tags></div><span
                    ...
                    <div class="product-modal-content"><span class="product-category" data-product-modal-category></span>
                        ...
                        <div class="product-modal-price" data-product-modal-price data-default-price></div>
                        <form class="product-modal-form js-submit-once" data-add-to-cart method="post"
                            ...
                            <div class="product-variant-picker" data-product-modal-variant-wrap hidden><label for="modal_variant_id">{{ __('customer_ui.variant_label') }}</label><div class="product-variant-select"><select class="form-select" id="modal_variant_id" name="variant_id" data-product-modal-variant data-variant-placeholder="{{ __('customer_ui.variant_placeholder') }}"></select></div></div>
