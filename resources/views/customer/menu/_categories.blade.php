<nav class="d-flex flex-wrap gap-2" aria-label="{{ __('app.categories.title') }}">
    ...
    @foreach ($categories as $category)<a class="btn btn-sm btn-outline-secondary" href="{{ route('customer.menu.index', ['category' => $category->slug]) }}">{{ $category->name }}</a>@endforeach
</nav>
