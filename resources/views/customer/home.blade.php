...
@section('content')
    @php
        ...
        $homeBannerVideoPath = $homeSettings[\App\Services\SystemSetting\SystemSettingCatalog::SITE_BANNER_VIDEO] ?? null;
        $homeBannerVideoUrl = $homeBannerVideoPath ? Storage::disk('public')->url($homeBannerVideoPath) : null;
    @endphp
    <section class="hero" @if ($homeBannerUrl) style="--hero-background-image: url('{{ $homeBannerUrl }}')" @endif>
        @if ($homeBannerVideoUrl)
            <video class="hero-media" data-hero-video data-src="{{ $homeBannerVideoUrl }}"
                poster="{{ $homeBannerUrl ?: asset('images/beer-garden-hero.png') }}" muted loop playsinline
    ...
    @php
        $featuredCategoryName = $usesFeaturedTag
            ...
            : __('customer_ui.signature_eyebrow'));
        $signatureDesktopColumns = min(4, max(1, $products->count()));
    @endphp
    <section class="signature-showcase" id="mon-noi-bat" aria-labelledby="signature-heading" data-carousel
                        ...
                        @foreach ($products as $product)
                            @php
                                ...
                                $foodFallbacks = [
                                    ...
                                    $homeBannerUrl ?: asset('images/beer-garden-hero.png'),
                                    $homeBannerUrl ?: asset('images/beer-garden-hero.png'),
                                    asset('images/brand/grilled-feast.jpg'),
                                ];
                                $fallbackImage =
    ...
    @if ($latestPosts->isNotEmpty())
        @php
            $newsFallbackImages = [
                ...
                'promotion' => $homeBannerUrl ?: asset('images/beer-garden-hero.png'),
                        ...
                        <article @class(['news-story', 'news-story--lead' => $loop->first])>
                            <a class="news-story-media" href="{{ route('customer.posts.show', $post) }}">
                                <img src="{{ $post->featured_image_url ?: ($newsFallbackImages[$post->category] ?? asset('images/brand/atmosphere-evening.jpg')) }}"
                                    alt="{{ $translatedPostTitle }}" loading="lazy" decoding="async">
                            </a>
