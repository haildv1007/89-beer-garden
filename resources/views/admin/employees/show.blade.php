...
@section('content')
    <div class="employee-page employee-profile-page">
        <a class="admin-back-link" href="{{ route('admin.employees.index') }}">← Danh sách nhân viên</a>
        <header class="employee-profile-header">
            ...
            <div><span
                    class="badge text-bg-{{ $employee->status->value === 'active' ? 'success' : 'secondary' }}">{{ __('employee.statuses.' . $employee->status->value) }}</span><a
                    class="btn btn-primary" href="{{ route('admin.employees.edit', $employee) }}">{{ __('app.edit') }}</a>
            </div>
        </header>
        ...
        @can('employee.disable')
            @if ($employee->status->value === 'active')
                <form method="post" action="{{ route('admin.employees.disable', $employee) }}"
                    onsubmit="return confirm('{{ __('employee.employees.disable_confirm') }}')">@csrf @method('patch')<button
                        class="btn btn-outline-danger">{{ __('employee.employees.disable') }}</button></form>
            @endif
        @endcan
        <section class="employee-profile-card employee-account-card">
            ...
            @if ($employee->user)
                <dl class="row">
                    <dt class="col-sm-3">{{ __('employee.fields.email') }}</dt>
                    <dd class="col-sm-9">{{ $employee->user->email }}</dd>
                    <dt class="col-sm-3">{{ __('employee.fields.status') }}</dt>
                    <dd class="col-sm-9">{{ __('employee.statuses.' . $employee->user->status) }}</dd>
                    <dt class="col-sm-3">{{ __('employee.roles.current') }}</dt>
                    <dd class="col-sm-9"><span class="badge text-bg-dark">{{ $employee->user->role->name }}</span></dd>
                </dl>
                @can('permission.assign')
                    <form class="row g-2" method="post" action="{{ route('admin.employees.roles.update', $employee) }}">@csrf
                        @method('patch')<div class="col-sm-6"><select class="form-select" name="role_id">
                        ...
                        <div class="col-sm-auto"><button class="btn btn-warning">{{ __('employee.roles.update') }}</button>
                        </div>
                    </form>
                @endcan
            @else
                    ...
                    <form method="post" action="{{ route('admin.employees.accounts.store', $employee) }}"
                        ...
                        <div class="mb-3"><label class="form-label"
                            ...
                            @error('role_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div><button class="btn btn-primary">{{ __('employee.accounts.create') }}</button>
                    </form>
                @endcan
            @endif
        </section>
