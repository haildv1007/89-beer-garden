...
@section('content')
                        ...
                        @else
                            ...
                            @can('payment.complete')
                                <form method="post"
                                    ...
                                    data-fulfillment-payment
                                    data-bank-transfer-url="{{ route($fulfillmentRoutes . '.bank-transfer.activate', $fulfillmentOrder) }}"
                                    data-payment-status-url="{{ route($fulfillmentRoutes . '.payment-status', $fulfillmentOrder) }}"
                                    data-invoice-url="{{ route($fulfillmentRoutes . '.invoice', $fulfillmentOrder) }}">@csrf<div><label>Phương thức đã thu</label><select
                                    ...
                                    @if ($fulfillmentOrder->fulfillment_type === 'pickup')
                                        <div class="billing-bank-transfer" data-bank-transfer-fields hidden>
                                            <a class="btn btn-outline-primary" target="_blank" rel="noopener"
                                                href="{{ route(($adminContext ?? false ? 'admin' : 'pos') . '.customer-display') }}">Mở màn hình khách</a>
                                            <button class="btn btn-outline-primary" type="button"
                                                data-bank-transfer-activate>Hiển thị QR trên màn hình khách</button>
                                            <small data-bank-transfer-message>QR có hiệu lực trong 30 phút.</small>
                                        </div>
                                    @endif
            ...
            @foreach ($fulfillmentOrder->items as $item)
                <article>
                    ...
                    <div class="admin-order-item-name">
                        <strong>{{ $item->product_name }}@if($item->variant_name) — {{ $item->variant_name }}@endif</strong><small>{{ $item->note ?: 'Không có ghi chú món' }}</small>
                    ...
                    <div class="fulfillment-editor__items">
                                    ...
                                    <select class="form-select" name="items[{{ $index }}][product_id]" required>
                                        @foreach ($products as $product)
                                            <option value="{{ $product->id }}" @selected($item->product_id === $product->id)
                                                ...
                                                {{ $product->name }} · {{ $product->display_price }}</option>
                                        @endforeach
                                    </select>
                                    <select class="form-select" name="items[{{ $index }}][variant_id]"
                                        data-variant-select @disabled($products->firstWhere('id', $item->product_id)?->availableVariants->isEmpty())
                                        @if($products->firstWhere('id', $item->product_id)?->availableVariants->isEmpty()) hidden @else required @endif>
                                        @foreach($products->firstWhere('id', $item->product_id)?->availableVariants ?? [] as $variant)
                                            <option value="{{ $variant->id }}" @selected($item->product_variant_id === $variant->id)>{{ $variant->name }} · {{ number_format($variant->price) }} ₫</option>
                                        @endforeach
                                    </select>
                        ...
                        <template data-fulfillment-item-template>
                            <div class="fulfillment-editor__item"><select class="form-select" data-field="product_id"
                                    ...
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}" data-variants='@json($product->availableVariants->map->only(["id", "name", "price"]))'>{{ $product->name }} ·
                                            {{ $product->display_price }}</option>
                                    @endforeach
                                </select><select class="form-select" data-field="variant_id" data-variant-select hidden disabled></select><input class="form-control" data-field="quantity" type="number" min="1"
    ...
    <script>
        const refreshFulfillmentVariant = row => {
            ...
            variant.innerHTML = variants.map(item => `<option value="${item.id}">${item.name} · ${Number(item.price).toLocaleString('vi-VN')} ₫</option>`).join('');
            variant.hidden = variants.length === 0; variant.disabled = variants.length === 0; variant.required = variants.length > 0;
        };
        document.addEventListener('change', event => {
            if (event.target.matches('.fulfillment-editor__item [name$="[product_id]"]')) refreshFulfillmentVariant(event.target.closest('.fulfillment-editor__item'));
        });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const method = form.querySelector('[data-payment-method]'),
                ...
                bankFields = form.querySelector('[data-bank-transfer-fields]'),
                activate = form.querySelector('[data-bank-transfer-activate]'),
                bankMessage = form.querySelector('[data-bank-transfer-message]'),
                total = {{ (int) $fulfillmentOrder->total_amount }};
            const refresh = () => {
                const cash = method.value === 'cash';
                field.hidden = !cash;
                received.disabled = !cash;
                if (bankFields) bankFields.hidden = cash;
                change.textContent = new Intl.NumberFormat('vi-VN').format(Math.max(0, Number(received.value ||
                    0) - total)) + ' ₫'
            };
            activate?.addEventListener('click', async () => {
                activate.disabled = true;
                bankMessage.textContent = 'Đang tạo QR...';
                try {
                    const response = await fetch(form.dataset.bankTransferUrl, {
                        method: 'POST',
                        headers: {Accept: 'application/json', 'X-CSRF-TOKEN': form.querySelector('[name="_token"]').value}
                    });
                    const data = await response.json();
                    if (!response.ok) throw new Error(data.message || Object.values(data.errors || {})[0]?.[0] || 'Không thể tạo QR.');
                    bankMessage.textContent = 'QR đã hiển thị trên màn hình khách và có hiệu lực trong 30 phút.';
                } catch (error) {
                    bankMessage.textContent = error.message;
                } finally {
                    activate.disabled = false;
                }
            });
            const checkPayment = async () => {
                if (method.value !== 'bank_transfer' || document.hidden) return;
                try {
                    const response = await fetch(form.dataset.paymentStatusUrl, {headers: {Accept: 'application/json'}});
                    const data = await response.json();
                    if (data.paid) window.location.assign(form.dataset.invoiceUrl);
                    else if (data.expired && bankMessage) bankMessage.textContent = 'Mã QR đã hết hạn. Bấm hiển thị QR để tạo mã mới.';
                } catch (_) {}
            };
            setInterval(checkPayment, 2500);
