...
@section('content')
    ...
    <header class="billing-page-header">
        ...
        <div class="billing-header-actions"><span
            ...
            @unless ($paid)<a class="btn btn-outline-primary" target="_blank" rel="noopener" href="{{ route($billingPrefix . '.customer-display') }}">Mở màn hình khách</a>@endunless
            @if ($paid)
                ...
                <header>
                    ...
                    @unless ($paid)
                        <form method="post"
                            ...
                            @csrf<button class="btn btn-sm btn-outline-primary">Tính lại hóa đơn</button></form>
                    @endunless
                </header>
                                ...
                                @foreach ($order->items as $item)
                                    <div class="billing-line">
                                        <div class="billing-line-product">
                                            <span class="billing-line-product__image">
                                            ...
                                            <div><strong>{{ $item->product_name }}@if($item->variant_name) — {{ $item->variant_name }}@endif</strong>
                                                @if ($item->note)<small>Ghi chú: {{ $item->note }}</small>@endif
                                            </div>
                ...
                @can('payment.complete')
                    <section class="billing-card billing-payment-card">
                        <header>
                            <div>
                                <h2>Thu tiền</h2>
                                <p>Cần thu <strong>{{ number_format($bill->total_amount, 0, ',', '.') }} ₫</strong></p>
                            </div>
                        </header>
                        <form method="post" action="{{ route($billingPrefix . '.bills.payments.complete', $bill) }}"
                            data-payment-form data-bank-transfer-url="{{ route($billingPrefix . '.bills.bank-transfer.activate', $bill) }}"
                            data-payment-status-url="{{ route($billingPrefix . '.bills.payment-status', $bill) }}"
                            data-invoice-url="{{ route($billingPrefix . '.bills.invoice', $bill) }}">@csrf<label for="method">Phương thức thanh toán</label><select class="form-select"
                            ...
                            <div class="billing-bank-transfer" data-bank-transfer-fields hidden>
                                <button class="btn btn-outline-primary w-100" type="button" data-bank-transfer-activate>Hiển thị QR trên màn hình khách</button>
                            ...
                            <label class="billing-confirm-check"><input type="checkbox" name="confirmed_received" value="1"
                                    @checked(old('confirmed_received'))><span data-payment-confirm-label>Đã nhận đủ
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const bankFields = form.querySelector('[data-bank-transfer-fields]');
            const activate = form.querySelector('[data-bank-transfer-activate]');
            const bankMessage = form.querySelector('[data-bank-transfer-message]');
            const confirmLabel = form.querySelector('[data-payment-confirm-label]');
            const total = {{ (int) $bill->total_amount }};
            const refresh = () => {
                const cash = method.value === 'cash';
                fields.hidden = !cash;
                received.disabled = !cash;
                bankFields.hidden = method.value !== 'bank_transfer';
                confirmLabel.firstChild.textContent = method.value === 'bank_transfer'
                    ? 'Đã kiểm tra và nhận đủ '
                    : 'Đã nhận đủ ';
            ...
            activate.addEventListener('click', async () => {
                activate.disabled = true; bankMessage.textContent = 'Đang tạo QR...';
                try {
                    const response = await fetch(form.dataset.bankTransferUrl, {method:'POST', headers:{Accept:'application/json','X-CSRF-TOKEN':form.querySelector('[name="_token"]').value}});
                    const data = await response.json();
                    if (!response.ok) throw new Error(data.message || 'Không thể tạo QR.');
                    bankMessage.textContent = 'QR đã được gửi sang màn hình khách và có hiệu lực trong 30 phút.';
                } catch (error) { bankMessage.textContent = error.message; } finally { activate.disabled = false; }
            });
            const checkPayment = async () => {
                if (method.value !== 'bank_transfer' || document.hidden) return;
                try { const response = await fetch(form.dataset.paymentStatusUrl,{headers:{Accept:'application/json'}}); const data=await response.json(); if(data.paid) window.location.assign(form.dataset.invoiceUrl); else if(data.expired) bankMessage.textContent='Mã QR đã hết hạn. Bấm hiển thị QR để tạo mã mới.'; } catch (_) {}
            };
