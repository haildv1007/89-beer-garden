...
class AuthenticationTest extends TestCase
{
    ...
    public function test_invalid_credentials_are_rejected_with_a_generic_error(): void
    {
        ...
        $event = SecurityEvent::query()->where('event', 'login_failed')->sole();
        $this->assertNull($event->user_id);
        $this->assertSame(
            hash_hmac('sha256', strtolower($user->email), (string) config('app.key')),
            $event->identifier_hash,
        );
        $this->assertStringNotContainsString(strtolower($user->email), (string) $event->toJson());
    }
    ...
    public function test_login_is_rate_limited_after_five_failures(): void
    {
        ...
        $key = 'login:identity:'.strtolower($user->email);
        $this->assertTrue(RateLimiter::tooManyAttempts($key, 5));
        $this->assertDatabaseHas('security_events', ['event' => 'login_locked']);
        $this->assertGuest();
    }
    ...
    public function test_security_headers_are_added_without_blocking_same_origin_print_frames(): void
    {
