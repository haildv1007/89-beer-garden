...
class CustomerRegistrationTest extends TestCase
{
    ...
    public function test_guest_can_view_registration_form(): void
    {
        $this->get(route('customer.registration.create'))
            ->assertOk()->assertViewIs('customer.registration.create')
            ->assertSee('name="password"', false)->assertSee('name="_token"', false);
    }
    ...
    public function test_valid_registration_atomically_creates_and_authenticates_customer_account(): void
    {
        ...
        $response = $this->post(route('customer.registration.store'), [
            'name' => 'Lan Nguyen', 'email' => '  LAN@EXAMPLE.COM ', 'phone' => '0901000000',
    ...
    {
        User::factory()->forRole(Role::where('code', 'customer')->firstOrFail())->create(['email' => 'taken@example.com']);
        $password = 'short';

        $response = $this->from(route('customer.registration.create'))->post(route('customer.registration.store'), [
            'name' => 'Invalid', 'email' => 'TAKEN@EXAMPLE.COM', 'phone' => '0902',
            'password' => $password, 'password_confirmation' => 'different',
        ])->assertRedirect(route('customer.registration.create'))->assertSessionHasErrors(['email', 'password']);
    ...
    {
        Event::listen('eloquent.creating: '.Customer::class, fn () => throw new RuntimeException('Customer insert failed.'));
    ...
    public function test_authenticated_user_cannot_open_or_submit_registration(): void
    {
        ...
        $this->post(route('customer.registration.store'), $this->validPayload('second@example.com'))->assertRedirect(route('customer.home'));
    ...
    public function test_registration_rejects_forged_identity_fields(): void
    {
        $payload = $this->validPayload('forged@example.com') + [
            ...
            'status' => User::STATUS_DISABLED, 'user_id' => 999, 'note' => 'internal',
        ];

        $this->post(route('customer.registration.store'), $payload)
            ->assertSessionHasErrors(['role_id', 'status', 'user_id', 'note']);
    ...
    public function test_registration_is_rate_limited(): void
    {
        for ($attempt = 0; $attempt < 5; $attempt++) {
            $this->post(route('customer.registration.store'), [])->assertSessionHasErrors(['name', 'email', 'password']);
        }
    ...
    {
        return [
            'name' => 'Customer', 'email' => $email, 'phone' => '0900000000',
