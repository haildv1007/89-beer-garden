...
class CustomerRegistrationTest extends TestCase
{
    ...
    public function test_guest_can_view_registration_form(): void
    {
        $this->get(route('customer.registration.create'))
            ...
            ->assertSee('required autocomplete="email"', false)
    ...
    public function test_phone_and_name_are_optional_but_email_and_password_are_required(): void
    {
        $payload = $this->validPayload('required@example.com');
        ...
        $this->assertSame('Khách hàng', $user->customer->name);
        $this->assertNull($user->customer->phone);
    }

    public function test_registration_without_email_is_rejected_without_partial_data(): void
    {
        $payload = $this->validPayload('unused@example.com');
        unset($payload['email']);

        $this->post(route('customer.registration.store'), $payload)->assertSessionHasErrors('email');
    ...
    public function test_registration_is_rate_limited(): void
    {
        for ($attempt = 0; $attempt < 5; $attempt++) {
            $this->post(route('customer.registration.store'), [])->assertSessionHasErrors(['email', 'password']);
        }
