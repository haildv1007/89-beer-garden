...
class ReservationRequestTest extends TestCase
{
    ...
    public function test_guest_can_render_form_and_create_pending_reservation_without_selecting_table(): void
    {
        ...
        $this->get(route('customer.reservations.confirmation'))
            ...
            ->assertSee($payload['phone'])
            ->assertSee(now()->addDay()->format('d/m/Y'))
            ->assertSee(substr($payload['reservation_time'], 0, 5))
            ->assertSee(__('reservation.people', ['count' => $payload['party_size']]))
            ->assertSee('Private request note');
    ...
    public function test_guest_can_submit_reservation_with_durable_preorder_snapshot(): void
    {
        ...
        $this->get(route('customer.reservations.create', ['preorder' => 1]))
            ...
            ->assertSee('Ít cay, không hành')
            ->assertDontSee(__('customer_ui.required_note'))
            ->assertSee('300.000 ₫');
        ...
        $this->assertSame('Ít cay, không hành', $reservation->preorder->items->sole()->note);
    ...
    public function test_at_table_cart_handoff_option_can_be_selected(): void
    {
