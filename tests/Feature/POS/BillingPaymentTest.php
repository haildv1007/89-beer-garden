...
class BillingPaymentTest extends TestCase
{
    ...
    public function test_open_billing_aggregates_all_orders_and_non_cancelled_historical_items(): void
    {
        ...
        $product->forceFill(['image_url' => '/images/historical-beer.jpg'])->save();
        ...
        $this->get(route('pos.bills.show', $bill))
            ->assertOk()
            ->assertSee('Historical Beer')
            ->assertSee('/images/historical-beer.jpg')
            ->assertSee('Tính lại hóa đơn')
            ->assertDontSee('Đang chờ')
            ->assertDontSee('← Phiên')
    ...
    public function test_dine_in_bank_transfer_is_shown_on_customer_display_and_completed_by_sepay(): void
    {
        foreach ([
            ...
            $setting = SystemSetting::query()->where('key', $key)->first() ?? new SystemSetting();
            $setting->forceFill(['key' => $key, 'value' => $value, 'type' => 'string'])->save();
        }
        $secret = SystemSetting::query()->where('key', SystemSettingCatalog::SEPAY_WEBHOOK_API_KEY)->first() ?? new SystemSetting();
        $secret->forceFill(['key' => SystemSettingCatalog::SEPAY_WEBHOOK_API_KEY, 'value' => Crypt::encryptString('dine-in-test-secret'), 'type' => 'string'])->save();
        $staff = $this->user('staff');
        $bill = $this->openedBill($staff, 125000);

        $this->post(route('pos.bills.bank-transfer.activate', $bill))->assertOk()->assertJson(['success' => true]);
        $bill->refresh();
        $this->assertNotNull($bill->payment_reference);
        $this->assertTrue($bill->payment_expires_at->isFuture());
        $this->getJson(route('pos.customer-display.status'))->assertOk()->assertJson([
            'active' => true, 'paid' => false, 'amount' => 125000,
        ])->assertJsonPath('reference', $bill->payment_reference);

        $this->withHeader('Authorization', 'Apikey dine-in-test-secret')->postJson(route('webhooks.sepay'), [
            'id' => 987654,
            'transactionDate' => now()->format('Y-m-d H:i:s'),
            'accountNumber' => '1234567890',
            'code' => $bill->payment_reference,
            'content' => $bill->payment_reference.' thanh toan',
            'transferType' => 'in',
            'transferAmount' => 125000,
            'referenceCode' => 'FT987654',
        ])->assertOk();
        ...
        $this->getJson(route('pos.customer-display.status'))->assertOk()->assertJson(['paid' => true]);
        $this->get(route('pos.dining-sessions.show', $bill->diningSession))
            ->assertOk()
            ->assertSee('Xem hóa đơn')
            ->assertSee(route('pos.bills.invoice', $bill));
        $this->get(route('pos.bills.invoice', $bill))
            ->assertOk()
            ->assertSee('Nội dung chuyển khoản')
            ->assertSee($bill->payment_reference)
            ->assertSee('FT987654');
    }
