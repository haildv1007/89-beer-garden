...
class ReservationManagementTest extends TestCase
{
    ...
    public function test_confirmed_reservation_becomes_late_can_be_filtered_and_still_checked_in(): void
    {
        Carbon::setTestNow('2026-09-22 19:10:00');
        ...
        try {
            ...
            $late = $this->reservation(ReservationStatus::Confirmed, [
                'reservation_code' => 'RSV-LATE',
                'reservation_date' => '2026-09-22',
                'reservation_time' => '19:00',
            ]);
            $onTime = $this->reservation(ReservationStatus::Confirmed, [
                'reservation_code' => 'RSV-ON-TIME',
                'reservation_date' => '2026-09-22',
                'reservation_time' => '19:08',
            ]);

            $this->actingAs($staff)
                ->get(route('pos.reservations.index', ['status' => ReservationStatus::Late->value]))
                ->assertOk()
                ->assertSee('RSV-LATE')
                ->assertDontSee('RSV-ON-TIME')
                ->assertSee('Trễ check-in');

            $this->assertSame(ReservationStatus::Late, $late->fresh()->status);
            $this->assertSame(ReservationStatus::Confirmed, $onTime->fresh()->status);

            $this->post(route('pos.reservations.check-in', $late), ['table_id' => $table->id])->assertRedirect();
            $this->assertSame(ReservationStatus::CheckedIn, $late->fresh()->status);
        } finally {
            Carbon::setTestNow();
        }
    }

    public function test_staff_can_cancel_unchecked_reservation_with_audit_and_preorder_cleanup(): void
    {
        $staff = $this->user('staff', true);
        $reservation = $this->reservation(ReservationStatus::Confirmed);
        $preorder = FulfillmentOrder::query()->forceCreate([
            'order_code' => 'NQ-CANCEL-001',
        ...
        $this->actingAs($staff)
            ->patch(route('pos.reservations.cancel', $reservation), ['reason' => ' Khách đổi kế hoạch '])
            ->assertRedirect();

        $reservation->refresh();
        $this->assertSame(ReservationStatus::Cancelled, $reservation->status);
        $this->assertSame('Khách đổi kế hoạch', $reservation->cancellation_reason);
        $this->assertSame($staff->employee->id, $reservation->cancelled_by_employee_id);
        $this->assertNotNull($reservation->cancelled_at);
        $this->assertSame(FulfillmentOrder::STATUS_REJECTED, $preorder->fresh()->status);
        $this->assertSame('Khách đổi kế hoạch', $preorder->fresh()->rejection_reason);

        $this->patch(route('pos.reservations.cancel', $reservation), ['reason' => 'Hủy lần nữa'])
            ->assertSessionHasErrors('reservation');
    }
