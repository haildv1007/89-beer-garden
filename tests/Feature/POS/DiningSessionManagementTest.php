...
class DiningSessionManagementTest extends TestCase
{
    ...
    public function test_admin_session_workspace_shows_value_and_supports_safe_service_edits(): void
    {
        ...
        $this->get(route('admin.dining-sessions.show', $session))
            ...
            ->assertSee('Nhật ký phiên')
            ->assertSee('Mở phiên phục vụ')
    ...
    public function test_staff_can_transfer_an_active_session_to_an_available_table_with_audit(): void
    {
        $admin = $this->user('admin');
        $source = $this->table(4, RestaurantTableStatus::Occupied);
        $target = $this->table(6);
        $reservation = $this->reservation(ReservationStatus::CheckedIn, 3);
        $reservation->forceFill(['table_id' => $source->id])->save();
        $session = $this->diningSession($source, $admin->employee);
        $session->forceFill(['reservation_id' => $reservation->id, 'guest_count' => 3])->save();

        $this->actingAs($admin)
            ->get(route('admin.dining-sessions.show', $session))
            ->assertOk()
            ->assertSee('Đổi bàn');

        $this->patch(route('admin.dining-sessions.transfer-table', $session), [
            'table_id' => $target->id,
            'reason' => 'Khách muốn chuyển vào trong',
        ])->assertRedirect()->assertSessionHasNoErrors();

        $this->assertSame($target->id, $session->fresh()->table_id);
        $this->assertSame($target->id, $reservation->fresh()->table_id);
        $this->assertSame(RestaurantTableStatus::Cleaning, $source->fresh()->runtime_status);
        $this->assertSame(RestaurantTableStatus::Occupied, $target->fresh()->runtime_status);
        $this->assertDatabaseHas('dining_session_table_transfers', [
            'dining_session_id' => $session->id,
            'from_table_id' => $source->id,
            'to_table_id' => $target->id,
            'transferred_by_employee_id' => $admin->employee->id,
            'reason' => 'Khách muốn chuyển vào trong',
        ]);

        $occupiedTarget = $this->table(8, RestaurantTableStatus::Occupied);
        $this->patch(route('admin.dining-sessions.transfer-table', $session), [
            'table_id' => $occupiedTarget->id,
            'reason' => 'Thử đổi sai',
        ])->assertSessionHasErrors('table_id');
        $this->assertSame($target->id, $session->fresh()->table_id);
    }

    public function test_staff_can_cancel_an_empty_session_but_not_a_session_with_live_items(): void
    {
        $admin = $this->user('admin');
        $table = $this->table(5, RestaurantTableStatus::Occupied);
        $reservation = $this->reservation(ReservationStatus::CheckedIn, 5);
        $reservation->forceFill(['table_id' => $table->id, 'checked_in_at' => now()])->save();
        $session = $this->diningSession($table, $admin->employee);
        $session->forceFill(['reservation_id' => $reservation->id])->save();

        $this->actingAs($admin)->patch(route('admin.dining-sessions.cancel', $session), [
            'cancellation_reason' => 'Khách đổi ý và rời quán',
        ])->assertRedirect()->assertSessionHasNoErrors();

        $session->refresh();
        $this->assertSame(DiningSessionStatus::Cancelled, $session->status);
        $this->assertSame($admin->employee->id, $session->cancelled_by_employee_id);
        $this->assertSame('Khách đổi ý và rời quán', $session->cancellation_reason);
        ...
        $this->get(route('admin.dining-sessions.show', $session))
            ->assertOk()->assertSee('Hủy phiên phục vụ')->assertSee('Khách đổi ý và rời quán');

        $busyTable = $this->table(4, RestaurantTableStatus::Occupied);
        $busySession = $this->diningSession($busyTable, $admin->employee);
        $category = Category::query()->firstOrCreate(['slug' => 'cancel-test'], [
            'name' => 'Test', 'status' => Category::STATUS_ACTIVE, 'sort_order' => 1,
        ]);
        $product = Product::query()->forceCreate([
            'category_id' => $category->id, 'name' => 'Món đang dùng', 'slug' => 'cancel-item-test',
            'price' => 50000, 'status' => Product::STATUS_ACTIVE, 'is_available' => true,
        ]);
        $order = Order::query()->forceCreate([
            'order_code' => 'ORD-CANCEL-BLOCK', 'dining_session_id' => $busySession->id,
            'source' => 'staff', 'ordered_at' => now(),
        ]);
        OrderItem::query()->forceCreate([
            'order_id' => $order->id, 'product_id' => $product->id, 'product_name' => $product->name,
            'quantity' => 1, 'unit_price' => 50000, 'line_total' => 50000, 'status' => 'waiting',
        ]);

        $this->patch(route('admin.dining-sessions.cancel', $busySession), [
            'cancellation_reason' => 'Không được phép',
        ])->assertSessionHasErrors('dining_session');
        $this->assertSame(DiningSessionStatus::Active, $busySession->fresh()->status);
        $this->assertSame(RestaurantTableStatus::Occupied, $busyTable->fresh()->runtime_status);
    }
