...
class PaymentConcurrencyTest extends TestCase
{
    ...
    public function test_concurrent_last_voucher_usage_allows_only_one_bill_to_complete(): void
    {
        ...
        $voucher = Voucher::query()->forceCreate(['code' => 'LAST', 'name' => 'Last use', 'discount_type' => Voucher::TYPE_FIXED,
            'discount_value' => 1000, 'min_order_amount' => 0, 'start_at' => now()->subHour(), 'end_at' => now()->addHour(),
            'usage_limit' => 1, 'used_count' => 0, 'status' => Voucher::STATUS_ACTIVE]);
    ...
    public function test_payment_racing_additional_order_never_omits_committed_items(): void
    {
        ...
        $this->assertContains($results, [[0, 0], [0, 2]]);
        $expected = (int) OrderItem::query()->whereHas('order', fn ($query) => $query->where('dining_session_id', $session->id))
            ->where('status', '!=', OrderItemStatus::Cancelled->value)->sum('line_total');
    ...
    public function test_payment_racing_item_cancellation_or_transition_has_no_deadlock_or_drift(): void
    {
        foreach (['cancel', 'prepare'] as $action) {
            ...
            $expected = (int) OrderItem::query()->whereHas('order', fn ($query) => $query->where('dining_session_id', $session->id))
                ->where('status', '!=', OrderItemStatus::Cancelled->value)->sum('line_total');
            $this->assertSame($expected, $bill->fresh()->subtotal);
            $this->assertSame($expected, Payment::query()->where('bill_id', $bill->id)->where('status', 'success')->sole()->amount);
        }
    }
    ...
    {
        $opener = Employee::query()->forceCreate(['employee_code' => 'OPEN-'.$suffix, 'name' => 'Opener', 'status' => EmployeeStatus::Active]);
        $table = RestaurantTable::query()->forceCreate(['code' => 'T-'.$suffix, 'name' => 'Table', 'capacity' => 4,
            'runtime_status' => RestaurantTableStatus::Occupied, 'is_active' => true]);
        $session = DiningSession::query()->forceCreate(['session_code' => 'DS-'.$suffix, 'table_id' => $table->id,
            'opened_by_employee_id' => $opener->id, 'status' => DiningSessionStatus::Active, 'started_at' => now(), 'guest_count' => 2]);
        if ($product === null) {
            $category = Category::query()->forceCreate(['name' => 'Category', 'slug' => 'category-'.$suffix,
                'status' => Category::STATUS_ACTIVE, 'sort_order' => 1]);
            $product = Product::query()->forceCreate(['category_id' => $category->id, 'name' => 'Product', 'slug' => 'product-'.$suffix,
                'price' => 10000, 'status' => Product::STATUS_ACTIVE, 'is_available' => true]);
        }
    ...
    private function process(string $action, int $targetId, int $actorId, int $extraId = 0): Process
    {
        $script = <<<'PHP'
            ...
            try {
                ...
                match ($argv[1]) {
                    'pay' => app(App\Services\Payment\CompletePaymentService::class)->complete(App\Models\Bill::findOrFail((int) $argv[2]), $actor, 'cash', null),
                    'fail' => app(App\Services\Payment\RecordFailedPaymentService::class)->record(App\Models\Bill::findOrFail((int) $argv[2]), $actor, 'cash', null, 'Concurrent failure'),
                    'order' => app(App\Services\Order\CreateOrderService::class)->create(App\Models\DiningSession::findOrFail((int) $argv[2]), $actor, [['product_id' => (int) $argv[4], 'quantity' => 1]], null),
                    'cancel' => app(App\Services\OrderItem\CancelOrderItemService::class)->cancelWaiting(App\Models\OrderItem::findOrFail((int) $argv[2]), $actor, 'Concurrent'),
                    'prepare' => app(App\Services\OrderItem\OrderItemTransitionService::class)->startPreparing(App\Models\OrderItem::findOrFail((int) $argv[2]), $actor),
                };
                exit(0);
        ...
        $process = new Process([PHP_BINARY, '-r', $script, $action, (string) $targetId, (string) $actorId,
            (string) $extraId, base_path()], base_path(), [
