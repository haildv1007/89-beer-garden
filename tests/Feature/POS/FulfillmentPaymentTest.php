...
class FulfillmentPaymentTest extends TestCase
{
    ...
    public function test_pickup_order_can_show_qr_on_customer_display_and_complete_from_sepay(): void
    {
        foreach ([
        ...
        $secret = SystemSetting::query()->where('key', SystemSettingCatalog::SEPAY_WEBHOOK_API_KEY)->first()
            ?? new SystemSetting();
        $secret->forceFill([
            'key' => SystemSettingCatalog::SEPAY_WEBHOOK_API_KEY,
            'value' => Crypt::encryptString('pickup-test-secret'),
            'type' => 'string',
        ])->save();
        $order = $this->createOrder('pickup', 'pay_on_receipt');

        $this->post(route('pos.fulfillment-orders.bank-transfer.activate', $order))
            ->assertOk()->assertJson(['success' => true]);
        $order->refresh();
        $this->assertSame(FulfillmentOrder::PAYMENT_BANK_TRANSFER, $order->payment_option);
        ...
        $this->getJson(route('pos.customer-display.status'))->assertOk()->assertJson([
            ...
            'table' => 'Đơn nhận tại quán',
        ])->assertJsonPath('reference', $order->payment_reference);

        $this->withHeader('Authorization', 'Apikey pickup-test-secret')->postJson(route('webhooks.sepay'), [
            'id' => 7654321,
            'transactionDate' => now()->format('Y-m-d H:i:s'),
            'accountNumber' => '1234567890',
            'code' => $order->payment_reference,
            'content' => $order->payment_reference.' thanh toan',
            'transferType' => 'in',
            'transferAmount' => $order->total_amount,
            'referenceCode' => 'FT7654321',
        ])->assertOk();
        ...
        $this->getJson(route('pos.fulfillment-orders.payment-status', $order))
