...
class OrderItemProcessingTest extends TestCase
{
    ...
    public function test_cancel_entire_round_records_each_item_and_one_kitchen_ticket(): void
    {
        $staff = $this->user('staff');
        $session = $this->diningSession();
        $first = $this->item($session, OrderItemStatus::Waiting);
        $second = $first->replicate();
        $second->save();

        $this->actingAs($staff)->patch(route('pos.orders.cancel', [$session, $first->order]), [
            'cancellation_reason' => '  Khách đổi ý  ',
        ])->assertRedirect();

        foreach ([$first, $second] as $item) {
            $this->assertSame(OrderItemStatus::Cancelled, $item->fresh()->status);
            $this->assertSame('Khách đổi ý', $item->fresh()->cancellation_reason);
            $this->assertSame($staff->employee->id, $item->fresh()->cancelled_by_employee_id);
        }
        $tickets = KitchenTicket::query()->where('source_type', 'dining_order')->where('source_id', $first->order_id)->get();
        $this->assertCount(1, $tickets);
        $this->assertSame(KitchenTicket::TYPE_CANCELLATION, $tickets->first()->type);
        $this->assertCount(2, $tickets->first()->payload['items']);
        $this->assertSame('HỦY CẢ LƯỢT: Khách đổi ý', $tickets->first()->payload['items'][0]['change']);
    }

    public function test_cancel_round_rejects_served_items_and_preparing_permission_without_partial_changes(): void
    {
        $staff = $this->user('staff');
        $session = $this->diningSession();
        $waiting = $this->item($session, OrderItemStatus::Waiting);
        $other = $waiting->replicate();
        $other->status = OrderItemStatus::Served;
        $other->save();
        $this->actingAs($staff)->patch(route('pos.orders.cancel', [$session, $waiting->order]), [
            'cancellation_reason' => 'Đổi ý',
        ])->assertSessionHasErrors('order');
        $this->assertSame(OrderItemStatus::Waiting, $waiting->fresh()->status);

        $other->status = OrderItemStatus::Preparing;
        $other->save();
        $this->patch(route('pos.orders.cancel', [$session, $waiting->order]), [
            'cancellation_reason' => 'Đổi ý',
        ])->assertForbidden();
        $this->assertSame(OrderItemStatus::Waiting, $waiting->fresh()->status);
    }
