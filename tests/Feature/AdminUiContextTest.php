...
class AdminUiContextTest extends TestCase
{
    ...
    public function test_sidebar_navigation_is_active_and_permission_guarded(): void
    {
        ...
        foreach (['admin.products.*', 'admin.customers.*', 'admin.inventory-items.*'] as $pattern) {
            $this->assertStringContainsString("request()->routeIs('{$pattern}')", $layout);
        }
        $this->assertStringNotContainsString("route('admin.roles.index')", $layout);
        $this->assertStringContainsString(
            "route('admin.roles.index')",
            file_get_contents(resource_path('views/admin/settings/partials/heading.blade.php')),
        );
    }
