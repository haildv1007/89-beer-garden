...
class InventoryManagementTest extends TestCase
{
    ...
    public function test_manager_can_create_optional_product_item_at_zero_and_update_metadata_only(): void
    {
        ...
        $this->actingAs($manager)->post(route('admin.inventory-items.store'), $this->itemPayload(' beer-01 ', null))
            ->assertRedirect();
        ...
        $this->put(route('admin.inventory-items.update', $item), $this->itemPayload('BEER-02', null, 5, 'inactive'))
            ->assertRedirect();
        $this->assertDatabaseHas('inventory_items', ['id' => $item->id, 'sku' => 'BEER-02',
            'minimum_stock' => 5, 'status' => 'inactive', 'current_stock' => 0]);
    }
    ...
    public function test_sku_product_uniqueness_and_product_validity_are_enforced(): void
    {
        ...
        $this->actingAs($manager)->post(route('admin.inventory-items.store'), $this->itemPayload('SKU-1', $product->id));
        $this->post(route('admin.inventory-items.store'), $this->itemPayload('sku-1', null))->assertSessionHasErrors('sku');
        $this->post(route('admin.inventory-items.store'), $this->itemPayload('SKU-2', $product->id))->assertSessionHasErrors('product_id');
        $deleted = $this->product('Deleted');
        $deleted->delete();
        $this->post(route('admin.inventory-items.store'), $this->itemPayload('SKU-3', $deleted->id))->assertSessionHasErrors('product_id');
        $inactive = $this->product('Inactive', Product::STATUS_INACTIVE);
        $this->post(route('admin.inventory-items.store'), $this->itemPayload('SKU-4', $inactive->id))->assertSessionHasErrors('product_id');
    ...
    {
        $manager = $this->user('manager');
        $this->actingAs($manager)->post(route('admin.inventory-items.store'), $this->itemPayload('FORGED', null) + ['current_stock' => 999])
            ->assertSessionHasErrors('current_stock');
        $item = $this->item('AUDIT', 10);
        $payload = ['type' => 'import', 'quantity' => 5, 'note' => '  Delivery  ', 'stock_before' => 99,
            'stock_after' => 999, 'created_by_employee_id' => 999, 'created_at' => now(), 'current_stock' => 999];
        $this->post(route('admin.inventory-items.movements.store', $item), $payload)
            ->assertSessionHasErrors(['stock_before', 'stock_after', 'created_by_employee_id', 'created_at', 'current_stock']);
        $this->assertDatabaseCount('stock_movements', 0);
        $this->post(route('admin.inventory-items.movements.store', $item), ['type' => 'import', 'quantity' => 5, 'note' => '  Delivery  ']);
    ...
    public function test_all_increase_and_decrease_types_create_a_valid_audit_chain(): void
    {
        ...
        foreach ([StockMovementType::Import, StockMovementType::AdjustmentIn, StockMovementType::Return] as $type) {
            $this->post(route('admin.inventory-items.movements.store', $item), ['type' => $type->value, 'quantity' => 3])->assertRedirect();
            $expected += 3;
        ...
        foreach ([StockMovementType::Export, StockMovementType::AdjustmentOut, StockMovementType::Damaged] as $type) {
            $this->post(route('admin.inventory-items.movements.store', $item), ['type' => $type->value, 'quantity' => 2])->assertRedirect();
            $expected -= 2;
            $this->assertSame($expected, $item->fresh()->current_stock);
        }
    ...
    public function test_invalid_quantity_insufficient_stock_and_overflow_are_rejected(): void
    {
        ...
        foreach ([0, -1, 1.5, 'five'] as $quantity) {
            $this->post(route('admin.inventory-items.movements.store', $item), ['type' => 'import', 'quantity' => $quantity])
                ->assertSessionHasErrors('quantity');
        }
        $this->post(route('admin.inventory-items.movements.store', $item), ['type' => 'export', 'quantity' => 6])
            ->assertSessionHasErrors('quantity');
        $item->forceFill(['current_stock' => PHP_INT_MAX])->save();
        $this->post(route('admin.inventory-items.movements.store', $item), ['type' => 'import', 'quantity' => 1])
            ->assertSessionHasErrors('quantity');
        $this->assertDatabaseCount('stock_movements', 0);
    ...
    public function test_persistence_failure_rolls_back_movement_and_stock(): void
    {
        ...
        Event::listen('eloquent.updating: '.InventoryItem::class, fn () => throw new RuntimeException('stock update failed'));
        $this->withoutExceptionHandling();
        try {
            $this->actingAs($manager)->post(route('admin.inventory-items.movements.store', $item), ['type' => 'import', 'quantity' => 5]);
    ...
    public function test_movement_history_is_immutable(): void
    {
        ...
        $this->actingAs($manager)->post(route('admin.inventory-items.movements.store', $item), ['type' => 'export', 'quantity' => 2]);
        $movement = StockMovement::query()->sole();
        try {
        ...
        $this->assertDatabaseHas('stock_movements', ['id' => $movement->id, 'quantity' => 2, 'stock_before' => 10, 'stock_after' => 8]);
    }
    ...
    public function test_low_stock_boundary_search_and_filters_work(): void
    {
        ...
        $this->actingAs($manager)->get(route('admin.inventory-items.index', ['low_stock' => 1]))
            ->assertOk()->assertSee('LOW-ZERO')->assertSee('LOW-EQUAL')->assertSee('INACTIVE')->assertDontSee('ENOUGH');
        $this->get(route('admin.inventory-items.index', ['q' => 'Special Linked']))->assertOk()->assertSee($lowZero->sku)->assertDontSee('ENOUGH');
        $this->get(route('admin.inventory-items.index', ['status' => 'inactive']))->assertOk()->assertSee('INACTIVE')->assertDontSee('LOW-EQUAL');
        $this->get(route('admin.inventory-items.show', $lowZero))->assertOk()->assertSee(__('inventory.low_stock_warning'));
    }
    ...
    public function test_deleted_or_inactive_item_rejects_new_movement(): void
    {
        ...
        $this->actingAs($manager)->post(route('admin.inventory-items.movements.store', $inactive), ['type' => 'export', 'quantity' => 1])
            ->assertSessionHasErrors('inventory_item');
        $deleted = $this->item('DELETED-MOVE', 10);
        $deleted->delete();
        $this->post(route('admin.inventory-items.movements.store', $deleted), ['type' => 'export', 'quantity' => 1])
            ->assertNotFound();
        $this->assertDatabaseCount('stock_movements', 0);
    }
    ...
    public function test_authorization_view_and_movement_permissions_are_independent(): void
    {
        ...
        foreach (['staff', 'kitchen', 'customer'] as $role) {
            $this->actingAs($this->user($role, $role !== 'customer'))->get(route('admin.inventory-items.index'))->assertForbidden();
        }
        $manager = $this->user('manager');
        ...
        $this->post(route('admin.inventory-items.movements.store', $item), ['type' => 'import', 'quantity' => 1])->assertForbidden();
    ...
    {
        return ['sku' => $sku, 'name' => 'Inventory '.$sku, 'unit' => 'unit', 'minimum_stock' => $minimum,
            'status' => $status, 'product_id' => $productId];
    }

    private function item(string $sku, int $stock, int $minimum = 0, string $status = 'active', ?Product $product = null): InventoryItem
    {
        return InventoryItem::query()->forceCreate(['product_id' => $product?->id, 'sku' => $sku, 'name' => $sku,
            'unit' => 'unit', 'current_stock' => $stock, 'minimum_stock' => $minimum, 'status' => $status]);
    }

    private function product(string $name, string $status = Product::STATUS_ACTIVE): Product
    {
        $category = Category::query()->first() ?? Category::query()->forceCreate(['name' => 'Category', 'slug' => 'inventory-category',
            'status' => Category::STATUS_ACTIVE, 'sort_order' => 1]);

        return Product::query()->forceCreate(['category_id' => $category->id, 'name' => $name,
            'slug' => 'inventory-'.fake()->unique()->numberBetween(1, 999999), 'price' => 10000,
    ...
    private function user(string $role, bool $employee = true): User
    {
        ...
        if ($employee) {
            Employee::query()->forceCreate(['user_id' => $user->id, 'employee_code' => 'INV-'.fake()->unique()->numberBetween(1, 999999),
                'name' => 'Inventory actor', 'status' => EmployeeStatus::Active]);
        }
