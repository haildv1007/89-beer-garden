...
class OperationalReportingTest extends TestCase
{
    ...
    public function test_successful_payment_is_counted_once_per_bill_and_payment_amount_is_authoritative(): void
    {
        ...
        Payment::query()->forceCreate([
            'payment_code' => 'PAY-DUPLICATE',
            'bill_id' => $sale['bill']->id,
            'processed_by_employee_id' => $employee->id,
            'method' => Payment::METHOD_CASH,
            'amount' => 999999,
            'status' => PaymentStatus::Success,
            'paid_at' => '2026-08-25 11:00:00',
        ]);
    ...
    public function test_additional_orders_in_one_paid_session_are_counted_separately_without_revenue_fanout(): void
    {
        ...
        $additional = Order::query()->forceCreate([
            'order_code' => 'ORD-ADDITIONAL',
            'dining_session_id' => $sale['order']->dining_session_id,
            'source' => 'customer',
            'ordered_at' => now(),
        ]);
    ...
    public function test_payment_history_filters_by_paid_at_instead_of_record_creation_time(): void
    {
        $sale = $this->sale($this->employee(), 42000, 'Beer', 1, OrderItemStatus::Served, '2026-08-25 10:00:00');
        $sale['payment']->forceFill(['created_at' => '2026-08-20 10:00:00'])->save();

        $today = app(PaymentHistoryQuery::class)->run(
            CarbonImmutable::parse('2026-08-25 00:00:00'),
            CarbonImmutable::parse('2026-08-25 23:59:59'),
            [],
        );
        $previous = app(PaymentHistoryQuery::class)->run(
            CarbonImmutable::parse('2026-08-20 00:00:00'),
            CarbonImmutable::parse('2026-08-20 23:59:59'),
            [],
        );

        $this->assertSame(42000, $today['summary']['total']);
        $this->assertSame(0, $previous['summary']['total']);
    }
    ...
    public function test_date_validation_default_presets_and_input_whitelist_are_enforced(): void
    {
        ...
        $this->get(
            route('admin.reports.index', ['preset' => 'custom', 'from' => '2026-08-26', 'to' => '2026-08-25']),
        )->assertSessionHasErrors('to');
        $this->get(
            route('admin.reports.index', ['preset' => 'custom', 'from' => '2025-01-01', 'to' => '2026-08-25']),
        )->assertSessionHasErrors('to');
    ...
    public function test_reservations_use_created_at_and_get_request_does_not_write(): void
    {
        ...
        $this->actingAs($manager)
            ->get(route('admin.reports.index', ['preset' => 'today']))
            ->assertOk()
            ->assertSee(__('report.reservation_basis'))
            ->assertSee(__('reservation.statuses.pending'));
    ...
    private function sale(
        ...
        $category = Category::query()->firstOrCreate(
            ['slug' => 'report'],
            ['name' => 'Report', 'status' => Category::STATUS_ACTIVE, 'sort_order' => 1],
        );
        $product = Product::query()->forceCreate([
            'category_id' => $category->id,
            'name' => $name,
            'slug' => 'p-'.fake()->unique()->numberBetween(1, 999999),
            'price' => intdiv($amount, $quantity),
        ...
        $bill = Bill::query()->forceCreate([
            'bill_code' => 'BILL-'.fake()->unique()->numberBetween(1, 999999),
            'dining_session_id' => $session->id,
            'subtotal' => $amount,
            'discount_amount' => 0,
            'total_amount' => $amount,
            'status' => $paymentStatus === PaymentStatus::Success ? BillStatus::Paid : BillStatus::Unpaid,
            'issued_at' => $paidAt,
        ]);
        $payment = Payment::query()->forceCreate([
            'payment_code' => 'PAY-'.fake()->unique()->numberBetween(1, 999999),
            'bill_id' => $bill->id,
            'processed_by_employee_id' => $employee->id,
            'method' => Payment::METHOD_CASH,
            'amount' => $amount,
