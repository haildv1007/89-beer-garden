...
class StockMovementConcurrencyTest extends TestCase
{
    ...
    public function test_concurrent_exports_allow_only_one_when_stock_is_insufficient_for_both(): void
    {
        ...
        $actor = $this->actor();
        $this->assertSame([0, 2], $this->race([
            $this->process($item, $actor, 'export', 7),
            $this->process($item, $actor, 'export', 7),
        ]));
    ...
    public function test_concurrent_import_and_export_do_not_lose_updates(): void
    {
        ...
        $actor = $this->actor();
        $this->assertSame([0, 0], $this->race([
            $this->process($item, $actor, 'import', 5),
            $this->process($item, $actor, 'export', 8),
        ]));
    ...
    public function test_double_submit_records_exactly_one_movement_per_successful_request(): void
    {
        ...
        $actor = $this->actor();
        $this->assertSame([0, 0], $this->race([
            $this->process($item, $actor, 'export', 2),
            $this->process($item, $actor, 'export', 2),
        ]));
    ...
    private function process(InventoryItem $item, User $actor, string $type, int $quantity): Process
    {
        ...
        $process = new Process([PHP_BINARY, '-r', $script, (string) $item->id, (string) $actor->id,
            $type, (string) $quantity, base_path()], base_path(), [
    ...
    {
        return InventoryItem::query()->forceCreate(['sku' => $sku, 'name' => $sku, 'unit' => 'unit',
            'current_stock' => $stock, 'minimum_stock' => 0, 'status' => InventoryItem::STATUS_ACTIVE]);
    }

    private function actor(): User
    {
        $user = User::factory()->forRole(Role::where('code', 'manager')->firstOrFail())->create();
        Employee::query()->forceCreate(['user_id' => $user->id, 'employee_code' => 'INV-RACE',
            'name' => 'Inventory racer', 'status' => EmployeeStatus::Active]);
