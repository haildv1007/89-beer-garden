...
class VoucherManagementTest extends TestCase
{
    ...
    public function test_manager_can_list_search_filter_create_update_and_soft_delete_unused_voucher(): void
    {
        ...
        $this->actingAs($manager)
            ->post(route('admin.vouchers.store'), $this->payload(' save10 ', 'Summer'))
            ->assertRedirect();
        ...
        $this->get(route('admin.vouchers.index', ['q' => 'save', 'status' => 'active']))
            ...
            ->assertSee('Summer');
        $this->put(
            route('admin.vouchers.update', $voucher),
            $this->payload('save20', 'Updated', 'percentage', 20),
        )->assertRedirect();
    ...
    public function test_validation_enforces_unique_code_time_amounts_percentage_and_server_owned_count(): void
    {
        ...
        $this->actingAs($admin)
            ->post(route('admin.vouchers.store'), $invalid)
    ...
    public function test_historical_voucher_cannot_be_edited_or_deleted(): void
    {
        ...
        $this->actingAs($manager)
            ->put(route('admin.vouchers.update', $voucher), $this->payload('CHANGED', 'Changed'))
    ...
    private function user(string $role): User
    {
        ...
        Employee::query()->forceCreate([
            'user_id' => $user->id,
            'employee_code' => 'V-'.fake()->unique()->numberBetween(1, 999999),
            'name' => 'Voucher actor',
            'status' => EmployeeStatus::Active,
        ]);
