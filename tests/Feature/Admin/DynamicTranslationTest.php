...
class DynamicTranslationTest extends TestCase
{
    ...
    public function test_latest_news_sidebar_uses_translated_post_titles(): void
    {
        $current = Post::query()->forceCreate([
            'title' => 'Bài hiện tại', 'slug' => 'bai-hien-tai', 'category' => 'food',
            'excerpt' => 'Giới thiệu', 'content' => 'Nội dung', 'content_format' => 'plain',
            'status' => Post::STATUS_PUBLISHED, 'published_at' => now()->subMinute(),
        ]);
        $latest = Post::query()->forceCreate([
            'title' => 'Tin mới nhất', 'slug' => 'tin-moi-nhat', 'category' => 'food',
            'excerpt' => 'Giới thiệu khác', 'content' => 'Nội dung khác', 'content_format' => 'plain',
            'status' => Post::STATUS_PUBLISHED, 'published_at' => now()->subMinutes(2),
        ]);
        Translation::query()->forceCreate([
            'translatable_type' => Post::class, 'translatable_id' => $latest->id,
            'field' => 'title', 'locale' => 'en', 'source_text' => $latest->title,
            'translated_text' => 'Latest story', 'source_hash' => hash('sha256', $latest->title),
            'source' => 'provider',
        ]);

        $html = $this->withSession(['locale' => 'en'])
            ->get(route('customer.posts.show', $current))->assertOk()->getContent();
        $sidebar = explode('<aside class="article-sidebar"', $html, 2)[1] ?? '';
        $this->assertStringContainsString('<h3>Latest story</h3>', $sidebar);
    }
