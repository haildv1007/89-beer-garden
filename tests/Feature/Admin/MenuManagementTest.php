...
class MenuManagementTest extends TestCase
{
    ...
    public function test_admin_menu_routes_enforce_context_employee_and_resource_permissions(): void
    {
        ...
        foreach (['customer', 'staff', 'kitchen'] as $role) {
            $this->actingAs($this->user($role, $role !== 'customer'))->get(route('admin.categories.index'))->assertForbidden();
        }
        ...
        $managerWithoutProducts->role->permissions()->detach(Permission::where('code', 'product.manage')->firstOrFail());
    ...
    public function test_category_with_products_is_protected_from_deletion(): void
    {
        ...
        Product::query()->create(['category_id' => $category->id, 'name' => 'Beer', 'slug' => 'protected-beer', 'price' => 10000, 'status' => 'active', 'is_available' => true]);
    ...
    public function test_product_crud_validates_business_fields_and_generic_update_cannot_change_price(): void
    {
        ...
        $category = $this->category();
        $payload = [
            'category_id' => $category->id, 'name' => 'Lager', 'slug' => 'lager',
            'description' => null, 'image_url' => null, 'price' => 50000,
            'status' => 'active', 'is_available' => '1',
        ];

        $this->actingAs($admin)->post(route('admin.products.store'), $payload)->assertRedirect();
        $product = Product::where('slug', 'lager')->firstOrFail();
        $this->post(route('admin.products.store'), $payload + ['slug' => 'lager'])
            ->assertSessionHasErrors('slug');
        $this->post(route('admin.products.store'), array_merge($payload, ['slug' => 'negative', 'price' => -1]))
            ->assertSessionHasErrors('price');

        $this->put(route('admin.products.update', $product), array_merge($payload, [
            'name' => 'Updated Lager', 'price' => 1, 'is_available' => '0', 'status' => 'inactive',
        ]))->assertSessionHasErrors('price');
        $this->assertSame(50000, $product->fresh()->price);

        unset($payload['price']);
        $this->put(route('admin.products.update', $product), array_merge($payload, [
            'name' => 'Updated Lager', 'is_available' => '0', 'status' => 'inactive',
        ]))->assertRedirect();
        $this->assertDatabaseHas('products', ['id' => $product->id, 'name' => 'Updated Lager', 'is_available' => false, 'status' => 'inactive']);
    ...
    public function test_price_endpoint_requires_both_permissions_and_preserves_historical_snapshot(): void
    {
        ...
        $product = Product::query()->create([
            'category_id' => $category->id, 'name' => 'Lager', 'slug' => 'lager',
            'price' => 50000, 'status' => 'active', 'is_available' => true,
        ]);
        $admin = $this->user('admin', true);
        ...
        $this->actingAs($withoutPrice)->patch(route('admin.products.price.update', $product), ['price' => 55000])->assertForbidden();
        $this->assertSame(50000, $product->fresh()->price);

        $this->actingAs($admin)->patch(route('admin.products.price.update', $product), ['price' => 55000])->assertRedirect();
    ...
    {
        $product = Product::query()->create(['category_id' => $this->category()->id, 'name' => 'Beer', 'slug' => 'beer', 'price' => 10000, 'status' => 'active', 'is_available' => true]);
        $manager = $this->user('manager', true);
        $manager->role->permissions()->detach(Permission::where('code', 'product.update-price')->firstOrFail());

        $this->actingAs($manager)->get(route('admin.products.show', $product))
            ->assertOk()->assertDontSee(__('app.products.update_price'));
    ...
    {
        return Category::query()->create(['name' => 'Beer', 'slug' => 'beer-'.fake()->unique()->numberBetween(1, 999999), 'status' => 'active', 'sort_order' => 0]);
    }
    ...
    private function user(string $role, bool $employee = false, EmployeeStatus $status = EmployeeStatus::Active): User
    {
        ...
        if ($employee) {
            Employee::forceCreate(['user_id' => $user->id, 'employee_code' => 'E-'.$user->id, 'name' => 'Employee', 'status' => $status]);
        }
    ...
    {
        $now = now();
        $tableId = DB::table('restaurant_tables')->insertGetId(['code' => 'T01', 'name' => 'Table 1', 'capacity' => 4, 'runtime_status' => 'occupied', 'is_active' => true, 'created_at' => $now, 'updated_at' => $now]);
        $sessionId = DB::table('dining_sessions')->insertGetId(['session_code' => 'S01', 'table_id' => $tableId, 'opened_by_employee_id' => $employee->id, 'status' => 'active', 'started_at' => $now, 'guest_count' => 2, 'created_at' => $now, 'updated_at' => $now]);
        $orderId = DB::table('orders')->insertGetId(['order_code' => 'O01', 'dining_session_id' => $sessionId, 'created_by_employee_id' => $employee->id, 'source' => 'staff', 'ordered_at' => $now, 'created_at' => $now, 'updated_at' => $now]);

        return DB::table('order_items')->insertGetId(['order_id' => $orderId, 'product_id' => $product->id, 'product_name' => $product->name, 'quantity' => 1, 'unit_price' => $price, 'line_total' => $price, 'status' => 'waiting', 'created_at' => $now, 'updated_at' => $now]);
    }
}
