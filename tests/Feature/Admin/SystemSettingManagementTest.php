...
class SystemSettingManagementTest extends TestCase
{
    ...
    public function test_admin_can_view_create_and_update_only_catalog_settings_with_audit(): void
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertSee('Cho phép đánh dấu “Không đến” sau (phút)')
            ->assertSee('Chuyển sang “Trễ check-in” sau (phút)')
            ->assertSee('Tạm khóa tài khoản sau số lần đăng nhập sai')
            ->assertSee('Tạm khóa đăng nhập từ IP sau số lần sai')
            ->assertSee('Thời gian tạm khóa đăng nhập (phút)')
            ->assertSee('không phải danh sách IP cho phép')
            ->assertDontSee('Khách tự gọi món')
    ...
    public function test_login_protection_settings_use_safe_ranges_and_defaults(): void
    {
        $service = app(UpdateSystemSettingService::class);
        $admin = $this->user('admin');
        $resolver = app(TypedSystemSettingResolver::class);

        $this->assertSame(['identity_attempts' => 5, 'ip_attempts' => 20, 'lock_minutes' => 5], $resolver->loginProtection());
        foreach (['0', '2', '21', '1.5', 'text'] as $value) {
            try {
                $service->update(SystemSettingCatalog::LOGIN_IDENTITY_ATTEMPTS, $value, $admin);
                $this->fail("Value {$value} must be rejected.");
            } catch (ValidationException) {
                $this->assertTrue(true);
            }
        }
        $service->update(SystemSettingCatalog::LOGIN_IDENTITY_ATTEMPTS, '8', $admin);
        $service->update(SystemSettingCatalog::LOGIN_IP_ATTEMPTS, '30', $admin);
        $service->update(SystemSettingCatalog::LOGIN_LOCK_MINUTES, '10', $admin);
        $this->assertSame(['identity_attempts' => 8, 'ip_attempts' => 30, 'lock_minutes' => 10], $resolver->loginProtection());
    }
    ...
    public function test_every_catalog_setting_has_a_working_update_route(): void
    {
        ...
        $values = [
            ...
            SystemSettingCatalog::LATE_CHECK_IN_MINUTES => '5',
            SystemSettingCatalog::DELIVERY_FEE => '30000',
            SystemSettingCatalog::LOGIN_IDENTITY_ATTEMPTS => '5',
            SystemSettingCatalog::LOGIN_IP_ATTEMPTS => '20',
            SystemSettingCatalog::LOGIN_LOCK_MINUTES => '5',
            ...
            SystemSettingCatalog::GEMINI_ENABLED => '1',
            SystemSettingCatalog::GEMINI_API_KEY => 'test-gemini-api-key-123456789',
            SystemSettingCatalog::GEMINI_MODEL => 'gemini-2.5-flash',
        ];
    ...
    public function test_tabbed_settings_save_a_group_and_replace_uploaded_images(): void
    {
        ...
        $this->get(route('admin.settings.index', ['tab' => 'connections']))
            ->assertOk()
            ->assertSee('Đăng nhập Google')
            ->assertSee('Dịch tự động')
            ->assertSee('Bật trợ lý AI Gemini')
            ->assertSee('Gemini 2.5 Flash')
            ->assertDontSee('tab=google_login', false)
            ->assertDontSee('tab=translation', false);
        ...
        $this->assertSame('scripts', array_key_last(app(SystemSettingCatalog::class)->groups()));
        ...
        $this->put(route('admin.settings.group.update', 'connections'), [
            'values' => [
                SystemSettingCatalog::GEMINI_ENABLED => '1',
                SystemSettingCatalog::GEMINI_API_KEY => 'test-gemini-api-key-123456789',
                SystemSettingCatalog::GEMINI_MODEL => 'gemini-3.6-flash',
            ],
        ])->assertRedirect(route('admin.settings.index', ['tab' => 'connections']));
        $this->assertDatabaseHas('system_settings', [
            'key' => SystemSettingCatalog::GEMINI_ENABLED,
            'value' => '1',
            'type' => 'boolean',
        ]);
        $this->assertSame('gemini-3.6-flash', app(TypedSystemSettingResolver::class)->gemini()['model']);
        $this->assertDatabaseMissing('system_settings', [
            'key' => SystemSettingCatalog::GEMINI_API_KEY,
            'value' => 'test-gemini-api-key-123456789',
        ]);
        ...
        $this->put(route('admin.settings.group.update', 'images'), [
            'images' => [
                SystemSettingCatalog::SITE_BANNER_VIDEO => UploadedFile::fake()->create(
                    'hero.mp4',
                    1024,
                    'video/mp4',
                ),
        ...
        $videoPath = SystemSetting::query()->where('key', SystemSettingCatalog::SITE_BANNER_VIDEO)->value('value');
    ...
    public function test_public_layout_uses_configured_seo_social_and_brand_values(): void
    {
        ...
        $values = [
            ...
            SystemSettingCatalog::SITE_LOGO => 'system-settings/site-logo.png',
            SystemSettingCatalog::SITE_BANNER => 'system-settings/site-banner.webp',
            SystemSettingCatalog::SITE_BANNER_VIDEO => 'system-settings/site-banner.mp4',
            SystemSettingCatalog::SITE_FAVICON => 'system-settings/favicon.png',
        ...
        $this->get(route('customer.home'))
            ...
            ->assertSee('data-src="/storage/system-settings/site-banner.mp4"', false)
            ->assertSee('name="description" content="Không gian vườn bia Việt, món ngon và những câu chuyện vui."', false)
            ->assertSeeInOrder(['property="og:title"', 'content="Hẹn nhau ở 89 Beer Garden"'], false)
            ->assertSee('href="/storage/system-settings/favicon.png"', false)
            ->assertSee('src="/storage/system-settings/site-logo.png"', false)
            ->assertSee("--hero-background-image: url('/storage/system-settings/site-banner.webp')", false)
        ...
        $this->get(route('customer.contact'))
            ->assertOk()
            ->assertSee('<title>'.__('customer_ui.contact_title').' - 89 Beer Garden</title>', false)
            ->assertDontSee(__('customer_ui.contact_title').' — 89 Beer Garden', false);
    }

    public function test_admin_can_manage_validated_public_contact_information_as_one_group(): void
    {
        $admin = $this->user('admin');

        $this->actingAs($admin)
            ->get(route('admin.settings.index', ['tab' => 'contact']))
            ->assertOk()
            ->assertSee('Liên hệ &amp; cửa hàng', false)
            ->assertSee('Địa chỉ cửa hàng')
            ->assertSee('Hotline')
            ->assertSee('Email liên hệ')
            ->assertSee('Facebook / Messenger')
            ->assertSee('Zalo')
            ->assertSee('Giờ mở cửa')
            ->assertSee('Liên kết bản đồ');
        ...
        $this->put(route('admin.settings.group.update', 'contact'), ['values' => $values])
            ->assertRedirect(route('admin.settings.index', ['tab' => 'contact']))
            ->assertSessionHasNoErrors();

        $this->assertSame(
            [
                'address' => $values[SystemSettingCatalog::CONTACT_ADDRESS],
                'phone' => $values[SystemSettingCatalog::CONTACT_PHONE],
                'email' => 'hello@89beergarden.vn',
                'facebook_url' => $values[SystemSettingCatalog::CONTACT_FACEBOOK_URL],
                'zalo_url' => $values[SystemSettingCatalog::CONTACT_ZALO_URL],
                'opening_hours' => $values[SystemSettingCatalog::OPENING_HOURS],
                'map_url' => $values[SystemSettingCatalog::MAP_URL],
            ],
            app(TypedSystemSettingResolver::class)->contactInformation(),
        );

        foreach ([
            SystemSettingCatalog::CONTACT_PHONE => 'not-a-phone',
            SystemSettingCatalog::CONTACT_EMAIL => 'not-an-email',
            SystemSettingCatalog::CONTACT_FACEBOOK_URL => 'https://evil.example.test/fake-facebook',
            SystemSettingCatalog::CONTACT_ZALO_URL => 'http://zalo.me/insecure',
            SystemSettingCatalog::MAP_URL => 'http://insecure.example.test/map',
        ] as $key => $invalid) {
            $this->put(route('admin.settings.group.update', 'contact'), ['values' => [$key => $invalid]])
                ->assertSessionHasErrors("values.{$key}");
        }
    }

    public function test_social_settings_accept_identifiers_and_domains_and_store_safe_urls(): void
    {
        $this->actingAs($this->user('admin'));

        $this->put(route('admin.settings.group.update', 'contact'), ['values' => [
            SystemSettingCatalog::CONTACT_FACEBOOK_URL => 'haileo1007',
            SystemSettingCatalog::CONTACT_ZALO_URL => '0398964982',
        ]])->assertRedirect()->assertSessionHasNoErrors();

        $contact = app(TypedSystemSettingResolver::class)->contactInformation();
        $this->assertSame('https://facebook.com/haileo1007', $contact['facebook_url']);
        $this->assertSame('https://zalo.me/0398964982', $contact['zalo_url']);

        $this->put(route('admin.settings.group.update', 'contact'), ['values' => [
            SystemSettingCatalog::CONTACT_FACEBOOK_URL => 'facebook.com/haileo1007',
        ]])->assertRedirect()->assertSessionHasNoErrors();
        $this->assertSame('https://facebook.com/haileo1007', app(TypedSystemSettingResolver::class)->contactInformation()['facebook_url']);
    }
