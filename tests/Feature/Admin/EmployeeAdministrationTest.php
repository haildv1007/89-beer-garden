...
class EmployeeAdministrationTest extends TestCase
{
    ...
    public function test_admin_can_link_an_existing_customer_account_and_preserve_customer_profile(): void
    {
        $admin = $this->user('admin', true);
        $customer = $this->user('customer');
        $customer->customer()->create(['name' => 'Khách cũ', 'email' => $customer->email]);
        $employee = $this->employee();
        $staffRole = Role::where('code', 'staff')->firstOrFail();

        $this->actingAs($admin)->post(route('admin.employees.accounts.link', $employee), [
            'existing_email' => strtoupper($customer->email),
            'role_id' => $staffRole->id,
        ])->assertRedirect(route('admin.employees.show', $employee));

        $this->assertSame($customer->id, $employee->fresh()->user_id);
        $this->assertSame($staffRole->id, $customer->fresh()->role_id);
        $this->assertSame('Khách cũ', $customer->customer->fresh()->name);
    }

    public function test_link_existing_account_rejects_another_employee_account(): void
    {
        $admin = $this->user('admin', true);
        $existingStaff = $this->user('staff', true);
        $employee = $this->employee();

        $this->actingAs($admin)->post(route('admin.employees.accounts.link', $employee), [
            'existing_email' => $existingStaff->email,
            'role_id' => Role::where('code', 'staff')->value('id'),
