...
class RestaurantTableManagementTest extends TestCase
{
    ...
    public function test_crud_validates_configuration_and_keeps_runtime_status_server_owned(): void
    {
        ...
        $this->actingAs($manager)->post(route('admin.restaurant-tables.store'), $payload + ['runtime_status' => 'occupied'])
        ...
        $this->put(route('admin.restaurant-tables.update', $table), array_merge($payload, ['capacity' => 0]))->assertSessionHasErrors('capacity');
        $this->put(route('admin.restaurant-tables.update', $table), array_merge($payload, ['name' => 'Forged', 'runtime_status' => 'cleaning']))
            ->assertSessionHasErrors('runtime_status');
        $this->assertSame(RestaurantTableStatus::Available, $table->fresh()->runtime_status);

        $this->put(route('admin.restaurant-tables.update', $table), array_merge($payload, ['name' => 'Patio Updated', 'capacity' => 6]))->assertRedirect();
        $this->assertDatabaseHas('restaurant_tables', ['id' => $table->id, 'name' => 'Patio Updated', 'capacity' => 6, 'runtime_status' => 'available']);
    }

    public function test_list_searches_and_filters_runtime_configuration_and_minimum_capacity(): void
    {
        $this->table()->forceFill(['name' => 'Main Large', 'capacity' => 8, 'location' => 'Hall'])->save();
        $this->table(RestaurantTableStatus::Cleaning)->forceFill(['name' => 'Patio Small', 'capacity' => 2, 'location' => 'Patio'])->save();
        $inactive = $this->table();
        $inactive->forceFill(['name' => 'Main Inactive', 'capacity' => 10, 'is_active' => false])->save();

        $this->actingAs($this->user('manager', true))->get(route('admin.restaurant-tables.index', [
            'q' => 'Main', 'status' => 'available', 'active' => '1', 'min_capacity' => 6,
        ]))->assertOk()->assertSee('Main Large')->assertDontSee('Patio Small')->assertDontSee('Main Inactive');
    }
    ...
    public function test_available_table_without_active_session_can_be_deactivated_and_soft_deleted(): void
    {
        $manager = $this->user('manager', true);
        $table = $this->table();
        $payload = ['code' => $table->code, 'name' => $table->name, 'capacity' => $table->capacity, 'location' => $table->location, 'is_active' => false];
    ...
    public function test_deactivate_and_delete_fail_closed_for_non_available_runtime_states(): void
    {
        ...
        foreach ([RestaurantTableStatus::Reserved, RestaurantTableStatus::Occupied, RestaurantTableStatus::Cleaning] as $status) {
            $table = $this->table($status);
            $payload = ['code' => $table->code, 'name' => $table->name, 'capacity' => $table->capacity, 'location' => $table->location, 'is_active' => false];

            $this->actingAs($manager)->put(route('admin.restaurant-tables.update', $table), $payload)->assertSessionHasErrors('table');
    ...
    private function table(RestaurantTableStatus $status = RestaurantTableStatus::Available): RestaurantTable
    {
        return RestaurantTable::query()->forceCreate([
            'code' => 'T-'.fake()->unique()->numberBetween(1, 999999), 'name' => 'Table', 'capacity' => 4,
            'location' => 'Main', 'runtime_status' => $status, 'is_active' => true,
        ]);
    }
    ...
    private function diningSession(RestaurantTable $table, DiningSessionStatus $status): DiningSession
    {
        ...
        return DiningSession::query()->forceCreate([
            ...
            'opened_by_employee_id' => $employee->id, 'status' => $status, 'started_at' => now(),
            'ended_at' => $status === DiningSessionStatus::Completed ? now() : null, 'guest_count' => 2,
        ]);
    }
