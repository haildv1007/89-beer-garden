...
Route::get('/lien-he', ContactController::class)->name('customer.contact');
...
Route::prefix('ai-chat')->name('customer.ai-chat.')->group(function (): void {
    Route::get('/state', [AIChatController::class, 'state'])->name('state');
    Route::post('/messages', [AIChatController::class, 'store'])
        ->middleware('throttle:ai-chat')
        ->block(5, 5)
        ->name('messages.store');
    Route::post('/reset', [AIChatController::class, 'reset'])
        ->middleware('throttle:ai-chat')
        ->block(5, 5)
        ->name('reset');
});
...
Route::post('/cart/items/batch', [CartController::class, 'storeBatch'])
    ->block(5, 5)
    ->name('customer.cart.items.batch-store');
Route::patch('/cart/items/{lineKey}', [CartController::class, 'update'])
    ->name('customer.cart.items.update');
Route::delete('/cart/items/{lineKey}', [CartController::class, 'destroy'])
...
Route::get('/cart/payment/{fulfillmentOrder}/status', [CartCheckoutController::class, 'paymentStatus'])
    ->middleware(['signed', 'throttle:30,1'])
    ->name('customer.cart.external-payment.status');
Route::get('/cart/payment/{fulfillmentOrder}/confirmed', [CartCheckoutController::class, 'paymentConfirmed'])
    ->middleware('signed')
    ->name('customer.cart.external-payment.confirmed');
...
Route::middleware('auth')->group(function () {
    Route::get('/orders', [\App\Http\Controllers\Customer\FulfillmentOrderHistoryController::class, 'index'])
        ->middleware('can:customer.order.view-own')
        ->name('customer.fulfillment-orders.index');
    Route::get('/orders/{fulfillmentOrder}', [\App\Http\Controllers\Customer\FulfillmentOrderHistoryController::class, 'show'])
        ->middleware('can:customer.order.view-own')
        ->name('customer.fulfillment-orders.show');
    Route::get('/payments', [OrderHistoryController::class, 'index'])
        ->middleware('can:customer.order.view-own')
        ->name('customer.orders.history');
    Route::get('/payments/orders/{fulfillmentOrder}', [OrderHistoryController::class, 'showFulfillment'])
        ->middleware('can:customer.order.view-own')
        ->name('customer.orders.history.fulfillment');
    Route::get('/payments/{bill}', [OrderHistoryController::class, 'show'])
        ->middleware('can:customer.order.view-own')
