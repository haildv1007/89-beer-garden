...
Route::post('fulfillment-orders/{fulfillmentOrder}/bank-transfer', [OperationalFulfillmentOrderController::class, 'activateBankTransfer'])
    ->middleware('can:payment.complete')
    ->name('fulfillment-orders.bank-transfer.activate');
Route::get('fulfillment-orders/{fulfillmentOrder}/payment-status', [OperationalFulfillmentOrderController::class, 'paymentStatus'])
    ->middleware('can:order.create')
...
Route::get('tables', [TableController::class, 'index'])
    ->middleware('can:table.view')
    ->name('tables.index');
Route::get('tables/customers/search', [DiningSessionController::class, 'searchCustomers'])
...
Route::patch('dining-sessions/{diningSession}/transfer-table', [OperationalDiningSessionController::class, 'transferTable'])
    ->middleware(['can:dining-session.view', 'can:table.operate'])
    ->name('dining-sessions.transfer-table');
Route::patch('dining-sessions/{diningSession}/cancel', [OperationalDiningSessionController::class, 'cancel'])
...
Route::get('bills/{bill}', [OperationalBillingController::class, 'show'])
    ->middleware('can:billing.view')
    ->name('bills.show');
Route::post('bills/{bill}/bank-transfer', [OperationalBillingController::class, 'activateBankTransfer'])
    ->middleware(['can:billing.view', 'can:payment.complete'])->name('bills.bank-transfer.activate');
Route::get('bills/{bill}/payment-status', [OperationalBillingController::class, 'paymentStatus'])
    ->middleware('can:billing.view')->name('bills.payment-status');
Route::get('customer-display', [OperationalBillingController::class, 'customerDisplay'])
    ->middleware('can:billing.view')->name('customer-display');
Route::get('customer-display/status', [OperationalBillingController::class, 'customerDisplayStatus'])
...
Route::post('dining-sessions/{diningSession}/orders', [OrderController::class, 'store'])
    ->middleware(['can:dining-session.view', 'can:order.create'])
    ->name('orders.store');
Route::patch('dining-sessions/{diningSession}/orders/{order}/cancel', [OrderController::class, 'cancel'])
