...
use App\Http\Requests\Admin\LinkEmployeeAccountRequest;
...
use App\Services\Access\LinkExistingEmployeeAccountService;
...
class EmployeeAccountController extends Controller
{
    ...
    public function link(
        ...
        Employee $employee,
        LinkExistingEmployeeAccountService $service,
    ): RedirectResponse {
        $data = $request->validated();
        $role = Role::query()->employee()->findOrFail($data['role_id']);
        $service->link($employee, $data['existing_email'], $role);

        return redirect()
            ->route('admin.employees.show', $employee)
            ->with('success', __('employee.accounts.linked'));
    }
}
