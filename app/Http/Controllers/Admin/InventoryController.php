...
class InventoryController extends Controller
{
    public function index(InventoryItemIndexRequest $request): View
    {
        ...
        $items = InventoryItem::query()->with('product:id,name')
    ...
    public function update(UpdateInventoryItemRequest $request, InventoryItem $inventoryItem, ManageInventoryItemService $service): RedirectResponse
    {
    ...
    {
        abort_unless(request()->user()?->can('inventory.view')
            && request()->user()?->can('inventory.stock-movement.create'), 403);
    ...
    public function storeMovement(StoreStockMovementRequest $request, InventoryItem $inventoryItem, AdjustStockService $service): RedirectResponse
    {
        $data = $request->validated();
        $service->record($inventoryItem, $request->user(), StockMovementType::from($data['type']),
            (int) $data['quantity'], $data['note'] ?? null);

        return redirect()->route('admin.inventory-items.show', $inventoryItem)->with('success', __('inventory.movement_created'));
    }
