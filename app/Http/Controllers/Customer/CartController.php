...
use App\Http\Requests\Customer\StoreCartBatchRequest;
...
class CartController extends Controller
{
    ...
    public function store(StoreCartItemRequest $request, CustomerCartService $cart): RedirectResponse|JsonResponse
    {
        ...
        $product = Product::with(['category', 'variants'])->findOrFail((int) $data['product_id']);
        $cart->add($request, $product, (int) $data['quantity'], $data['note'] ?? null, isset($data['variant_id']) ? (int) $data['variant_id'] : null);
    ...
    public function storeBatch(StoreCartBatchRequest $request, CustomerCartService $cart): RedirectResponse|JsonResponse
    {
        $cart->addBatch($request, $request->validated('items'));

        if ($request->expectsJson()) {
            return response()->json(
                $this->miniCartPayload($request, $cart) + [
                    'message' => __('recommendation.cart.added'),
                ],
            );
        }

        return back()->with('success', __('recommendation.cart.added'));
    }
    ...
    public function update(
        ...
        $data = $request->validated();
        $cart->update($request, $lineKey, (int) $data['quantity'], $data['note'] ?? null);

        if ($request->expectsJson()) {
            $rows = $cart->rows($request);
            $row = collect($rows)->firstWhere('line_key', $lineKey);
    ...
    public function destroy(Request $request, string $lineKey, CustomerCartService $cart): RedirectResponse|JsonResponse
    {
        $cart->remove($request, $lineKey);
