...
class CartCheckoutController extends Controller
{
    ...
    public function payment(FulfillmentOrder $fulfillmentOrder, TypedSystemSettingResolver $settings): View
    {
        ...
        if ($bank !== null && $fulfillmentOrder->payment_reference === null) {
            $fulfillmentOrder->forceFill([
                'payment_reference' => $bank['prefix'].substr(preg_replace('/[^A-Za-z0-9]/', '', $fulfillmentOrder->order_code), -12),
        ...
        $transferContent =
            $bank === null
                ? null
                : $fulfillmentOrder->payment_reference;
    ...
    public function paymentStatus(FulfillmentOrder $fulfillmentOrder): JsonResponse
    {
        return response()->json([
            ...
            'redirect_url' => $fulfillmentOrder->payment_status === FulfillmentOrder::PAYMENT_PAID
                ? URL::temporarySignedRoute('customer.cart.external-payment.confirmed', now()->addMinutes(10), compact('fulfillmentOrder'))
                : null,
        ]);
    }

    public function paymentConfirmed(FulfillmentOrder $fulfillmentOrder): View
    {
        abort_unless($fulfillmentOrder->payment_status === FulfillmentOrder::PAYMENT_PAID, 404);
        $fulfillmentOrder->load('items');

        $view = $fulfillmentOrder->fulfillment_type === FulfillmentOrder::TYPE_DELIVERY
            ? 'customer.checkout.delivery-success'
            : 'customer.checkout.pickup-success';

        return view($view, [
            'orderCode' => $fulfillmentOrder->order_code,
            'paymentReceipt' => $fulfillmentOrder,
        ]);
    }
