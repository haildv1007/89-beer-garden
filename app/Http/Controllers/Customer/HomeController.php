...
class HomeController extends Controller
{
    public function __invoke(DynamicTranslationResolver $resolver): View
    {
        ...
        $taggedProducts = $publicProducts
            ->filter(fn (Product $product): bool => $product->is_available && $product->tags->contains('slug', 'hot'))
            ->shuffle()
            ->values();
        $usesFeaturedTag = $taggedProducts->isNotEmpty();

        $categorySearchKey = fn (Category $category): string => Str::slug($category->name.' '.$category->slug);
        $populatedCategoryIds = $publicProducts->pluck('category_id')->unique();
        $populatedCategories = $categories->whereIn('id', $populatedCategoryIds);
        $featuredCategory =
            $populatedCategories->first(fn (Category $category) => str_contains($categorySearchKey($category), 'mon-hot')) ??
            ($populatedCategories->first(fn (Category $category) => str_contains($categorySearchKey($category), 'mon-nuong')) ??
                $publicProducts->pluck('category')->filter()->first());

        $products = $usesFeaturedTag
            ? $taggedProducts
            : ($featuredCategory
        ...
        return view(
            'customer.home',
            compact('categories', 'products', 'featuredCategory', 'usesFeaturedTag', 'latestPosts', 'dynamicTranslations', 'newsCategoryLabels'),
        );
    }
}
