...
class ReservationController extends Controller
{
    ...
    public function store(
        ...
        $request->session()->flash('reservation_details', [
            'name' => $data['name'],
            'phone' => $data['phone'],
        ...
        if ($withPreorder) {
            $preorder = $reservation->preorder()->with('items')->firstOrFail();
            $request->session()->flash('reservation_preorder', [
                'items' => $preorder->items->map(fn ($item): array => [
    ...
    {
        abort_unless($request->session()->has(['reservation_code', 'reservation_details']), 404);

        return view('customer.reservations.confirmation', [
            'reservationCode' => $request->session()->get('reservation_code'),
            'reservationDetails' => $request->session()->get('reservation_details'),
            'reservationPreorder' => $request->session()->get('reservation_preorder'),
        ]);
    }
