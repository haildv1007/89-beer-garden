...
class StoreOrderRequest extends FormRequest
{
    ...
    public function after(): array
    {
        return [function (Validator $validator): void {
            $keys = collect($this->input('items', []))->map(fn (array $item): string => ($item['product_id'] ?? '').':'.($item['variant_id'] ?? 0));
            if ($keys->duplicates()->isNotEmpty()) {
                $validator->errors()->add('items', 'Không được gửi trùng món và biến thể.');
            }
        }];
    }
}
