...
class KitchenTicketService
{
    ...
    public function createCancellationForOrder(Order $order, Collection $items, Employee $employee, string $reason): KitchenTicket
    {
        $order->loadMissing('diningSession.table');
        $payload = $this->orderPayload($order, $items->map(fn (OrderItem $item) => [
            ...$this->itemPayload($item),
            'change' => 'HỦY CẢ LƯỢT: '.$reason,
        ])->all());
        $payload['cancellation_scope'] = 'order';

        return $this->create('dining_order', $order->id, KitchenTicket::TYPE_CANCELLATION,
            "order-cancellation:{$order->id}", $payload, $employee->id);
    }
