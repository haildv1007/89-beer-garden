...
class CustomerCartService
{
    ...
    public function add(Request $request, Product $product, int $quantity, ?string $note): void
    {
        ...
        $cart[(string) $product->id] = [
            'product_id' => $product->id, 'quantity' => $newQuantity,
    ...
    public function itemsForSubmit(Request $request): array
    {
        ...
        foreach ($cart as $key => $item) {
            if (! is_array($item) || ! isset($item['product_id'], $item['quantity'])
                ...
                || (int) $item['quantity'] < 1 || (int) $item['quantity'] > 1000
            ...
            $items[] = ['product_id' => (int) $item['product_id'], 'quantity' => (int) $item['quantity'], 'note' => $item['note'] ?? null];
        }
    ...
    public function rows(Request $request): array
    {
        ...
        $products = Product::withTrashed()->with(['category' => fn ($query) => $query->withTrashed()])
            ->whereIn('id', array_keys($cart))->get()->keyBy('id');

        return collect($cart)->map(function (array $item) use ($products): array {
            $product = $products->get((int) $item['product_id']);
            $available = $product !== null && ! $product->trashed() && $product->status === Product::STATUS_ACTIVE
                && $product->is_available && $product->category !== null && ! $product->category->trashed()
                && $product->category->status === 'active';
            $price = $product?->price;
            $safeTotal = $price !== null && $price <= intdiv(PHP_INT_MAX, (int) $item['quantity']);

            return $item + ['name' => $product?->name ?? __('customer_order.unavailable_product'),
                'price' => $price, 'line_total' => $safeTotal ? $price * $item['quantity'] : null,
    ...
    {
        if ($product->trashed() || $product->status !== Product::STATUS_ACTIVE || ! $product->is_available
            || $product->category === null || $product->category->trashed() || $product->category->status !== 'active') {
