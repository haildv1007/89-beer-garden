...
class GoogleCustomerLoginService
{
    ...
    public function login(array $profile): User
    {
        ...
        return DB::transaction(function () use ($profile): User {
            ...
            if ($user === null) {
            ...
            } elseif (mb_strtolower($user->email) !== $email) {
                throw ValidationException::withMessages([
                    'google' => 'Tài khoản Google không khớp với email đã liên kết.',
                ]);
                ...
                if ((int) $user->role_id !== (int) $customerRole->id && ! $user->hasActiveEmployee()) {
                    throw ValidationException::withMessages([
                        'google' => 'Tài khoản nhân viên chưa hoạt động hoặc chưa được liên kết với hồ sơ nhân viên.',
                    ]);
                }
